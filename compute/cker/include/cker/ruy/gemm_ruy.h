...
{
  static void Run(const MatrixParams<LhsScalar> &lhs_params, const LhsScalar *lhs_data,
...
// code in that case.
    ...
    constexpr ruy::Path kRuyPath = ruy::Path::kReference | ruy::Path::kStandardCpp;
#else
    constexpr ruy::Path kRuyPath = ruy::kAllPaths;
#endif

    ruy::Mul<kRuyPath>(ruy_lhs, ruy_rhs, ruy_spec, context->ruy_context(), &ruy_dst);
  }
};
