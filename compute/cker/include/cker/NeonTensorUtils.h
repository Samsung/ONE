    ...
    const float *per_channel_scale, const int32_t *input_offset, int32_t *row_sums)
{
  ...
  assert(n_batch * sizeof(float) <= padded_scaling_factors_size);
  assert(batch_round_up * sizeof(float) <= padded_scaling_factors_size);
  ...
  if (input_offset != nullptr)
  {
    ...
    assert(n_batch * sizeof(int32_t) <= padded_input_offset_size);
    assert(batch_round_up * sizeof(int32_t) <= padded_input_offset_size);
