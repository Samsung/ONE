...
#ifndef __NEURUN_BACKEND_ACL_COMMON_ACL_SUB_TENSOR_ANALYZER_H__
#define __NEURUN_BACKEND_ACL_COMMON_ACL_SUB_TENSOR_ANALYZER_H__
...
public:
  void visit(const ir::OpSequence &op_seq) override
  {
  ...
  void visit(const ir::operation::Concat &node) override
  {
    ...
    for (const auto &input_index : inputs)
    {
      ...
      _parent_map.emplace(input_index, acl_common::ParentInfo{output_index, _current_op_seq_layout,
                                                              coordinate_info});
...
private:
  ...
  ir::Layout _current_op_seq_layout{ir::Layout::UNKNOWN};
};
...
#endif // __NEURUN_BACKEND_ACL_COMMON_ACL_SUB_TENSOR_ANALYZER_H__
