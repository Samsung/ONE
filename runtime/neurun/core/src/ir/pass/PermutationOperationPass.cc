...
void PermutationOperationPass::changeToKeepLayout(const Operation &node)
{
  ...
  {
    auto &above_subg = _lowered_graph.op_seqs().at(op_seq_index);
    ...
    {
      const auto &below_op_seq_index =
          _lowered_graph.op_seqs().emplace(it->index, *it->node, above_subg.getLayout());
      auto &below_subg = _lowered_graph.op_seqs().at(below_op_seq_index);
  ...
  // Remove target operation from op_seq and insert the target operation to new op_seq
  {
    ...
    _lowered_graph.op_seqs().removeFromSubgraph(node_index);
    ...
    auto &new_subg = _lowered_graph.op_seqs().at(new_op_seq_index);
