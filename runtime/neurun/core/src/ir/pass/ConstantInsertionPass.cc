...
{
  const auto &subgraph_index = _lowered_graph.op_seqs().getOperation(node_index);
  const auto op_seq_lower_info = _lowered_graph.getLowerInfo(subgraph_index);
  ...
  for (const auto input : node.getInputs())
  {
    ...
    if (object.isConstant())
    {
      ...
      // Update op_seq
      if (_lowered_graph.op_seqs().at(subgraph_index).getInputs().contains(input))
      {
        _lowered_graph.op_seqs().at(subgraph_index).replaceInput(input, replaced_input);
      }
