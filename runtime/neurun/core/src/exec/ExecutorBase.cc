...
{
  auto build_itensor_list = [&](const neurun::ir::OperandIndexSequence &ind_seq) {
...
void ExecutorBase::execute(const IODescription &desc)
{
  ...
  for (uint32_t n = 0; n < _graph.getInputs().size(); ++n)
  {
    ...
    auto setter = [&](::neurun::backend::ITensor &tensor) { sources.at(n)->push(tensor); };
  ...
  for (uint32_t n = 0; n < _graph.getOutputs().size(); ++n)
  {
    ...
    auto getter = [&](::neurun::backend::ITensor &tensor) { sinks.at(n)->pull(tensor); };
