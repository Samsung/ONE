...
struct ISource
{
  ...
  virtual void push(::neurun::backend::ITensor &tensor) const = 0;
};
...
protected:
  void pushUnif(neurun::backend::ITensor &tensor) const
  {
...
public:
  void push(neurun::backend::ITensor &tensor) const override
  {
...
public:
  void push(neurun::backend::ITensor &tensor) const override { ITemplSource<T>::pushUnif(tensor); }
};
