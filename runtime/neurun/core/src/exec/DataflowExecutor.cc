...
void DataflowExecutor::emplaceToReadyJobs(const uint32_t &id)
{
  ...
  assert(job != nullptr);
  auto &subg = _lowered_graph->op_seqs().at(_job_to_op_seq[job->index()]);
      ...
      _profiling{false}
{
  ...
  std::unordered_map<ir::OpSequenceIndex, uint32_t> subgraph_to_job;
  op_seqs.iterate([&](const ir::OpSequenceIndex &op_seq_index, const ir::OpSequence &) {
    ...
    subgraph_to_job[op_seq_index] = next_job_index++;
  });
  ...
  op_seqs.iterate([&](const ir::OpSequenceIndex &op_seq_index, const ir::OpSequence &subg) {
    auto job_index = subgraph_to_job[op_seq_index];
...
void DataflowExecutor::executeImpl()
{
  ...
  while (!_ready_jobs.empty())
  {
    ...
    auto op_seq = &_lowered_graph->op_seqs().at(subgraph_index);
    const backend::Backend *backend =
        _lowered_graph->getLowerInfo()->operation.at(subgraph_index)->backend();
