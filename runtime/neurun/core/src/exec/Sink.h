...
struct ISink
{
  ...
  virtual void pull(::neurun::backend::ITensor &tensor) const = 0;
};
...
protected:
  void pullUnif(neurun::backend::ITensor &tensor) const
  {
...
public:
  void pull(neurun::backend::ITensor &tensor) const override { ITemplSink<T>::pullUnif(tensor); }
};
