...
std::vector<ir::OpSequenceIndex> Linear::linearize(const ir::LoweredGraph &lowered_graph)
{
  std::vector<ir::OpSequenceIndex> order;
  {
    ...
    // op_seqs can't access a op_seq by an operand so that input_to_subgs can offer it
    std::unordered_map<ir::OperandIndex, std::list<ir::OpSequenceIndex>> input_to_subgs;
    ...
    //         |
    //        [4]
    op_seqs.iterate([&](const ir::OpSequenceIndex &op_seq_idx, const ir::OpSequence &subg) {
    ...
    std::function<void(const ir::OpSequenceIndex &, const ir::OpSequence &)> dfs_recursive =
        [&](const ir::OpSequenceIndex &index, const ir::OpSequence &subg) -> void {
      if (visited[index])
        return;
      visited[index] = true;
...
{
  {
    const auto &toString = [](const neurun::backend::Backend *backend) {
      assert(backend);
      std::string str;
