...
{
template <typename T>
static void Init(const neurun::ir::Operand &model_obj, neurun::backend::ITensor &obj,
                 const bool copy,
                 const neurun::ir::Layout frontend_layout = neurun::ir::Layout::UNKNOWN)
{
  ...
  obj.access([&](::neurun::backend::ITensor &tensor) {
    switch (shape.rank())
    {
      case 0:
      {
      ...
      case 2:
      {
        ...
        {
          ::neurun::ir::Coordinates coords{i, 0};
      ...
      case 3:
      {
        ...
        for (auto i = 0; i < shape.dim(0); ++i)
        {
          ...
          {
            ::neurun::ir::Coordinates coords{i, j, 0};
        ...
        for (auto i = 0; i < shape.dim(0); ++i)
        {
          for (auto j = 0; j < shape.dim(1); ++j)
          {
            for (auto k = 0; k < shape.dim(2); ++k)
            {
              if (copy)
              {
                ::neurun::ir::Coordinates coords{i, j, k, 0};
                memcpy(tensor.buffer() + tensor.calcOffset(coords),
              ...
              else
              {
                ...
                {
                  const auto coords = ::neurun::ir::convertCoordinates(
...
template <typename T>
void copyInit(const neurun::ir::Operand &model_obj, neurun::backend::ITensor &obj)
{
  Init<T>(model_obj, obj, true);
}

template <typename T>
void permuteInit(const neurun::ir::Operand &model_obj, neurun::backend::ITensor &obj,
