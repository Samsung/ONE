...
#ifndef __NEURUN_IR_LOWERED_GRAPH_H__
#define __NEURUN_IR_LOWERED_GRAPH_H__
...
public:
  ...
  const operation::LowerInfo *getLowerInfo(const OpSequenceIndex &subg_index) const;
  void setLowerInfo(const OpSequenceIndex &subg_index,
                    std::unique_ptr<operation::LowerInfo> &&lower_info);
  void removeLowerInfo(const OpSequenceIndex &subg_index);
...
private:
  ...
  void dumpLowerInfo();
  bool mergeable(const OpSequenceIndex &subg_index, const OperationIndex &node_index,
                 Layout layout);
  OpSequenceIndex appendFreshSingleOpSubgraph(const OperationIndex &node_index,
...
#endif // __NEURUN_IR_LOWERED_GRAPH_H__
