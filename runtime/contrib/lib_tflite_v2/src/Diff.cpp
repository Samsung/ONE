...
{
  assert(TfLiteInterpreterGetOutputTensorCount(interp) ==
         TfLiteInterpreterGetOutputTensorCount(nnapi));

  bool all_matched = true;

  using Comparator = std::function<bool(int id, TfLiteInterpreter *, TfLiteInterpreter *)>;

  std::map<TfLiteType, Comparator> comparators;

  comparators[kTfLiteUInt8] = [this](int id, TfLiteInterpreter *interp, TfLiteInterpreter *nnapi) {
    const auto expected = nnfw::tflite::TensorView<uint8_t>::makeOutputView(interp, id);
    const auto obtained = nnfw::tflite::TensorView<uint8_t>::makeOutputView(nnapi, id);

    return compareSingleTensorView(expected, obtained, id);
  };

  comparators[kTfLiteInt32] = [this](int id, TfLiteInterpreter *interp, TfLiteInterpreter *nnapi) {
    const auto expected = nnfw::tflite::TensorView<int32_t>::makeOutputView(interp, id);
    const auto obtained = nnfw::tflite::TensorView<int32_t>::makeOutputView(nnapi, id);

    return compareSingleTensorView(expected, obtained, id);
  };

  comparators[kTfLiteFloat32] = [this](int id, TfLiteInterpreter *interp,
                                       TfLiteInterpreter *nnapi) {
    const auto expected = nnfw::tflite::TensorView<float>::makeOutputView(interp, id);
    const auto obtained = nnfw::tflite::TensorView<float>::makeOutputView(nnapi, id);

    return compareSingleTensorView(expected, obtained, id);
  };

  comparators[kTfLiteBool] = [this](int id, TfLiteInterpreter *interp, TfLiteInterpreter *nnapi) {
    const auto expected = nnfw::tflite::TensorView<bool>::makeOutputView(interp, id);
    const auto obtained = nnfw::tflite::TensorView<bool>::makeOutputView(nnapi, id);
  ...
  {
    auto interp_tensor = TfLiteInterpreterGetOutputTensor(interp, output_idx);
    assert(interp_tensor->type == TfLiteInterpreterGetOutputTensor(nnapi, output_idx));

    auto it = comparators.find(interp_tensor->type);
    ...
    if (!comparator(output_idx, interp, nnapi))
    {
      all_matched = false;
    }
  }
