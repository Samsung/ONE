...
public:
   ...
   */
  void save(const std::string &path, TfLiteInterpreter *interp)
  {
    open(path);

    int log_index = 0;
    for (int32_t id = 0; id < TfLiteInterpreterGetInputTensorCount(interp); id++)
    {
      _outfile << "# input tensors" << std::endl;
      printInputTensor(interp, id, log_index++);
    }
    for (int32_t id = 0; id < TfLiteInterpreterGetOutputTensorCount(interp); id++)
    {
      _outfile << "# output tensors" << std::endl;
      printOutputTensor(interp, id, log_index++);
    }
    close();
  }
...
private:
  ...
  void printInputTensor(TfLiteInterpreter *interp, const int id, const int log_index)
  {
    const TfLiteTensor *tensor = TfLiteInterpreterGetInputTensor(interp, id);

    _outfile << "# input tensor name: " << tensor->name << std::endl;
    _outfile << "# tflite::interpreter.input_tensor(" << id << ") -> tensor_value_gen[" << log_index
             << "]" << std::endl;
  ...
  void printOutputTensor(TfLiteInterpreter *interp, const int id, const int log_index)
  {
    const TfLiteTensor *tensor = TfLiteInterpreterGetOutputTensor(interp, id);

    _outfile << "# output tensor name: " << tensor->name << std::endl;
