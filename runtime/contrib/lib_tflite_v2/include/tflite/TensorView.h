...
public:
   ...
   * @param[in] input_index The input tensor index
   * @return The new TensorView<T> object
   */
  static TensorView<T> makeInputView(TfLiteInterpreter *interp, int input_index)
  {
    auto tensor_ptr = TfLiteInterpreterGetInputTensor(interp, input_index);
    ...
    return TensorView<T>(shape, reinterpret_cast<T *>(tensor_ptr->data.raw));
  }
  static TensorView<T> makeOutputView(TfLiteInterpreter *interp, int output_index)
  {
    auto tensor_ptr = TfLiteInterpreterGetOutputTensor(interp, output_index);
