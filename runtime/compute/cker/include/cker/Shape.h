...
public:
  ...
  Shape(Shape const &other) : _size(other.DimensionsCount())
  {
    ...
    {
      _dims = std::vector<int32_t>(other.DimsData(), other.DimsData() + _size);
    }
    else
    {
  ...
  inline void ReplaceWith(Shape &&other)
  {
    ...
    if (_size <= kMaxSmallSize)
    {
      // Access the array inside the variant using std::get_if.
      auto *src = std::get_if<std::array<int32_t, kMaxSmallSize>>(&other._dims);
      auto *dst = std::get_if<std::array<int32_t, kMaxSmallSize>>(&_dims);
      if (src && dst)
      {
        std::copy(src->begin(), src->begin() + kMaxSmallSize, dst->begin());
      }
    }
    else
    {
