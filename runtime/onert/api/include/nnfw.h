...
extern "C"
{
   ...
   * Before inference, application has responsibility to set input tensor to set input data by
   * calling
...
#define NNFW_MAX_RANK (6)
   ...
   *
   * <p>Maximum rank is 6 (NNFW_MAX_RANK). And tensor's dimension value is filled in 'dims' field
   * from index 0. For example, if tensor's rank is 4, application can get dimension value from
   * dims[0], dims[1], dims[2], and dims[3]
   */
   ...
   *
   * This function can be called at any time after calling {@link nnfw_model_load_from_file}.
   * Changing input tensor's shape will cause shape inference for the model. There are two different
   * types of shape inference - static and dynamic. Which one to use is depend on the current state
   * of the session. When it is called after calling {@link nnfw_model_load_from_file} and before
   * calling {@link nnfw_prepare}, this info will be used when {@link nnfw_prepare}. And it will
   * perform static shape inference for all tensors. When it is called after calling {@link
   * nnfw_prepare} or even after {@link nnfw_run}, this info will be used when {@link nnfw_run}. And
   * the shapes of the tensors are determined on the fly. If this function is called many times for
   * the same index, it is overwritten.
   *
   * @param[in] session     Session to the input tensor info is to be set
   ...
   *
   * <p>This function must be called after calling {@link nnfw_run_asnyc}, and can be called only
   * once for a {@link nnfw_run_async} call.
   *
   * <p>When this function returns, it means that this session has finished the asynchronous run.
   * Then the user can safely use the output data.</p>
   *
   ...
   *
   * This function must be called after {@link nnfw_prepare}, \p buffer given to this function can
   * be reused for many inferences. \p length must be greater or equal than the operand requires. To
   ...
   *
   * This function must be called after {@link nnfw_prepare}, \p buffer given to this function can
   * be reused for many inferences. \p length must be greater or equal than the operand requires. An
   * output operand can have unspecified shape and deduced dynamically during the execution. You
   ...
   *
   * <p>After {@link nnfw_load_model_from_file} and before {@link nnfw_prepare} is invoked, it
   * returns tensor info in the model.</p>
   *
