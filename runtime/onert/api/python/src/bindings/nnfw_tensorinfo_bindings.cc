...
void bind_tensorinfo(py::module_ &m)
{
  ...
  py::class_<dtype>(m, "dtype", "Defines the type of the OneRT tensor.", py::module_local())
    ...
    .def_property_readonly(
      "name", [](const dtype &dt) { return dt.name(); }, "The name of the data type.")
    .def_property_readonly(
      "dtype", [](const dtype &dt) { return dt.py_dtype(); }, "A corresponding numpy data type.")
    .def_property_readonly(
      "itemsize", [](const dtype &dt) { return dt.itemsize(); },
