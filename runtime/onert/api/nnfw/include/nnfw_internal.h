...
/**
 * @brief Python-binding-only API to retrieve a read-only dynamic output buffer and its tensor info
 *
 ...
 * nnfw_set_prepare_config(session, NNFW_PREPARE_CONFIG_ENABLE_INTERNAL_OUTPUT_ALLOC, "true")
 ...
 */
NNFW_STATUS nnfw_get_dynamic_output(const nnfw_session *session, uint32_t index,
