...
NNFW_STATUS Session::prepare()
{
  ...
  if (!isStateModelLoaded())
  {
    if (isStateInitialized())
    {
      setLastErrorMessage("Error during model prepare : prepare should be called once");
    }
    else
    {
      setLastErrorMessage("Error during model prepare : Invalid state");
    }
    return NNFW_STATUS_INVALID_STATE;
  }
  ...
  {
    setLastErrorMessage("Error during model prepare : " + std::string(e.what()));
    return NNFW_STATUS_ERROR;
  }
...
const std::string &Session::get_last_error_message() const { return _last_error_message; }
                                      ...
                                      size_t length)
{
  ...
  {
    setLastErrorMessage(
      "Error during Session::train_set_output : buffer is NULL but length is not 0");
    return NNFW_STATUS_ERROR;
  }
