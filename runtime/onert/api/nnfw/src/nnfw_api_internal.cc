...
NNFW_STATUS nnfw_session::run_with_auto_compilation(const char *target, NNFW_CODEGEN_PREF pref)
{
  ...
  std::string target_str{target};
  if (_quant_manager->exportModelPath().empty() ||            
      _codegen_manager->exportModelPath().empty() ||
      target_str.empty() || 
  ...
  if (!file_quantized_model.good() && !file_compiled_model.good())
  {
    // Run float model and try to qunatize it
    {
      ...
      {
        // Currently insufficient buffer always means output buffer.
        std::cerr << "Error during nnfw_session::run_with_auto_compilation : " << e.what() << std::endl;
        return NNFW_STATUS_INSUFFICIENT_OUTPUT_SIZE;
      }
      catch (const std::exception &e)
      {
      ...
      if (_quant_manager->readyForQuantize())
      {
        ...
        {
          std::cerr << "Error during nnfw_session::run_with_auto_compilation in quantize operation: " << e.what() << std::endl;
          return NNFW_STATUS_ERROR;
        }
      }
    }
