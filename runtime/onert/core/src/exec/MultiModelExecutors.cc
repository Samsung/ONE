...
void MultiModelExecutors::createEdgeQuantLayers()
{
  ...
  for (const auto &pair : _edge_map)
  {
    ...
    // Assume that model's layout is always NHWC
    _edge_tensors[from_iodesc] =
      std::make_unique<backend::builtin::EdgeTensor>(from_info, from_layout);
  }
        ...
        for (const auto &to_iodesc : to_list)
        {
          ...
          if (from_tensor->data_type() != to_info.typeInfo().type())
          {
            ...
            auto type_aware_quant_tensor =
              std::make_unique<backend::builtin::EdgeTensor>(to_info, to_layout);
...
void MultiModelExecutors::CreatePkgIOTensors(const IODescription &desc)
{
  for (const auto &pkg_input : _model_edges->pkg_inputs)
  {
    ...
    _pkg_input_tensors[pkg_input] = std::make_unique<backend::builtin::UserTensor>(
      input_desc->info, input_desc->layout, reinterpret_cast<const uint8_t *>(input_desc->buffer),
      input_desc->size);
  }
  ...
  for (const auto &pair : _executors)
  {
    ...
    if (pkg_inputs.size() > 0)
    {
      ...
      for (const auto &pkg_input : pkg_inputs)
      {
        ...
        const auto &orig_info = executor->inputInfo(std::get<ir::IOIndex>(pkg_input).value());
        const auto orig_layout = executor->inputLayout(std::get<ir::IOIndex>(pkg_input).value());
        if (input_desc->info.typeInfo().type() != orig_info.typeInfo().type())
        {
          auto pkg_input_edge_tensor =
            std::make_unique<backend::builtin::EdgeTensor>(orig_info, orig_layout);
    ...
    {
      // We need to consider input description is changed
      _pkg_input_quant_layers[{model_index, subg_index}] = nullptr;
    }
    ...
    if (pkg_outputs.size() > 0)
    {
      ...
      for (const auto &pkg_output : pkg_outputs)
      {
        ...
        const auto &orig_info = executor->outputInfo(std::get<ir::IOIndex>(pkg_output).value());
        const auto &orig_layout = executor->outputLayout(std::get<ir::IOIndex>(pkg_output).value());
    ...
    {
      // We need to consider output description is changed
      _pkg_output_quant_layers[{model_index, subg_index}] = nullptr;
    }
  }
}
...
void MultiModelExecutors::execute(const ExecutionContext &ctx)
{
  ...
  for (auto model_index = ir::ModelIndex{0}; model_index.value() < model_count; model_index++)
  {
    ...
    for (uint32_t i = 0; i < output_size; i++)
    {
      ...
      else
      {
        ...
        for (const auto &to_iodesc : _edge_map[from_iodesc])
        {
          ...
          if (_edge_quant_tensors.find(to_iodesc) != _edge_quant_tensors.end())
          {
            _edge_quant_tensors.at(to_iodesc)->allocate_buffer();
    ...
    if (_pkg_input_quant_layers[{model_index, ir::SubgraphIndex{0}}] != nullptr)
    ...
    if (_pkg_output_quant_layers[{model_index, ir::SubgraphIndex{0}}] != nullptr)
