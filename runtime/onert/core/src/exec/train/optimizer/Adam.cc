...
using namespace onert::exec::train::optimizer;

//////////////////////////////////////////////////////
// This is an implementation of adam from original paper.
template <typename T>
void adam_origin(const ITensor &grad_tensor, const Adam::Property &props, double lr,
                 train::ITrainableTensor &trainable_tensor)
{
  const auto beta1 = props.beta1;
  const auto beta2 = props.beta2;
  const auto epsilon = props.epsilon;
  ...
  const auto shape = grad_tensor.getShape();
  // TODO Support for different shapes
  if (vv->getShape() != shape || vs->getShape() != shape || trainable_tensor.getShape() != shape)
  {
    throw std::runtime_error("Adam: Invalid shape");
  }
  ...
  auto rescaleGrad = [&](double v, double s) -> double {
...
void Adam::applyGradient(const UpdateFactors &factors) const
{
  ...
  switch (grad_tensor.data_type())
  {
    case ir::DataType::FLOAT32:
      adam_origin<float>(grad_tensor, _props, lr, trainable_tensor);
      break;
    default:
