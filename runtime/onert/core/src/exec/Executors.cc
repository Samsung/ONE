...
void Executors::checkSupportedMultimodel() const
{
  ...
  // Assumption: nnpkg inputs/outputs are unique.
  std::vector<ir::IODesc> pkg_io;
  pkg_io.insert(pkg_io.end(), _model_edges->pkg_inputs.begin(), _model_edges->pkg_inputs.end());
  pkg_io.insert(pkg_io.end(), _model_edges->pkg_outputs.begin(), _model_edges->pkg_outputs.end());
  if (pkg_io.size() != std::unordered_set<ir::IODesc>{pkg_io.begin(), pkg_io.end()}.size())
  {
    throw std::runtime_error{
      "Multi model execution does not support duplicated nnpkg inputs/outputs"};
  }
...
void Executors::InitPkgIO(const IODescription &desc)
{
  _pkg_inputs.resize(desc.inputs.size());
  _pkg_outputs.resize(desc.outputs.size());
  ...
  for (auto &pair : _executors)
  {
    ...
    for (const auto &pkg_input : pkg_inputs)
    {
      ...
      _pkg_inputs[input_pkg_index]->setUserTensor(
        reinterpret_cast<uint8_t *>(const_cast<void *>(input_desc->buffer)), input_desc->size);

      // Create EdgeTensor for nnpkg inputs if types are different
      ...
      if (input_desc->info.typeInfo().type() != input_tensor->orig_info().typeInfo().type())
      {
        ...
        src_tensors.emplace_back(_pkg_inputs[input_pkg_index].get());
        dst_tensors.emplace_back(_pkg_io_edge_tensors[pkg_input].get());
      }
    }
    ...
    src_tensors.clear();
    dst_tensors.clear();
    // Create EdgeTensor for nnpkg outputs
    for (const auto &pkg_output : pkg_outputs)
    {
      ...
      if (output_desc->info.typeInfo().type() != output_tensor->orig_info().typeInfo().type())
      {
        ...
        // Append type-aware quantization layer's inputs/outputs
        src_tensors.emplace_back(_pkg_io_edge_tensors[pkg_output].get());
        dst_tensors.emplace_back(_pkg_outputs[output_pkg_index].get());
      }
    }
...
void Executors::execute(const IODescription &desc)
{
  ...
  // TODO Create initializing type-aware quantization layers only once and recreate type-aware
  // quantization layers only if types change
  InitPkgIO(desc);
  ...
  for (auto model_index = ir::ModelIndex{0}; model_index.value() < model_count; model_index++)
  {
    ...
    {
      auto input_pkg_index = find_input_index(_model_edges->pkg_inputs, model_index,
      ...
      if (input_pkg_index != -1)
      {
        ...
        {
          inputs_inter[i] = _pkg_inputs[input_pkg_index].get();
        }
      }
      else
      {
    ...
    {
      auto output_pkg_index = find_output_index(_model_edges->pkg_outputs, model_index,
                                                ir::SubgraphIndex{0}, ir::IOIndex{i});
      if (output_pkg_index != -1)
      {
        ...
        {
          outputs_inter[i] = _pkg_outputs[output_pkg_index].get();
        }
      }
      else
      {
    ...
    for (uint32_t i = 0; i < input_size; i++)
    {
      ...
      if (input_pkg_index == -1)
      {
        ...
        else
        {
          ...
          if (_pkg_io_edge_tensors.find(to_iodesc) != _pkg_io_edge_tensors.end())
          {
            _pkg_io_edge_tensors[to_iodesc]->decrease_ref();
          }
        }
      }
    }
    ...
    for (uint32_t i = 0; i < output_size; i++)
    {
      ...
      // Assumption: nnpkg inputs/outputs are not duplicated with edges.
      if (_pkg_io_edge_tensors.find(from_iodesc) != _pkg_io_edge_tensors.end())
      {
        _pkg_io_edge_tensors[from_iodesc]->decrease_ref();
      }
    }
  }
}
