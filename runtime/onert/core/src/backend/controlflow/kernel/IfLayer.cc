...
void IfLayer::run()
{
  ...
  assert(_input_tensors.size() == subg_exec->getInputTensors().size());
  ...
  src_tensors.clear();
  dst_tensors.clear();
  assert(_output_tensors.size() == subg_exec->getOutputTensors().size());
  for (uint32_t i = 0; i < _input_tensors.size(); ++i)
  {
    ...
    {
      const auto &subg_output_index = subg_graph.getOutputs().at(i);
      if (subg_graph.getInputs().contains(subg_output_index))
      {
        // Add tensor from operation's inputs since some inputs of subgraph are not copied from
        // operation's inputs
        src_tensors.emplace_back(_input_tensors.at(i));
      }
      else
