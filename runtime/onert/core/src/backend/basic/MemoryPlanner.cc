...
// inline void BumpPlanner<Index>::claim(const Index &ind, size_t size)
// {
...
// //   - This claim() inserts key/value into _claim_table and the release() removes the key/value
// from
// //     _claim_table.
...
// //       it means the place at the offset has been already claimed(== can't claim now. need to
// find
// //       someplace new).
...
// // 2. In the loop for _claim_table, we can assume the current claim_base_offset value is bigger
// than
...
//           interfered_plans.emplace(_mem_plans[interference].offset,
//           _mem_plans[interference].size);
//       }
