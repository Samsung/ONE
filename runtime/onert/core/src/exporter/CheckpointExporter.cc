...
void DataBuffer::resize(uint32_t length, uint32_t total_size)
{
...
void DataBuffer::setOffset(uint32_t offset) { _cur_offset = offset; }

void DataBuffer::setData(const char *data, uint32_t size)
{
...
void DataBuffer::write(std::ofstream &ostream)
{
...
void CheckpointExporter::setTensorData(const std::unique_ptr<exec::Execution> &exec)
{
  ...
  const uint32_t offset = sizeof(_header);
...
// void CheckpointExporter::setAdamOptimizerData(const std::unique_ptr<onert::exec::Execution>
// &exec)
// {
//   // Reserved
//   // - optimizer data offset for 4 bytes * 2
...
//   assert(v_ptr == _optimizers.data() + m_offset_size + m_total_size + v_offset_size +
//   v_total_size);
