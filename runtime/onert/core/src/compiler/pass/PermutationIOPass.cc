                                           ...
                                           const ir::Layout &from_layout)
{
  ...
  if (operand.typeInfo().type() == ir::DataType::FLOAT32 && from_layout == ir::Layout::NHWC)
    return;

  // Update graph operand
  auto input_typeinfo = use_float ? ir::TypeInfo{ir::DataType::FLOAT32} : operand.typeInfo();
  auto input_shape = from_layout == ir::Layout::NCHW
                       ? ir::convertShape(operand.shape(), ir::Layout::NHWC, ir::Layout::NCHW)
                       : operand.shape();
  ...
  // Update use/def info
  _graph.operands().at(input_operand_index).setDef(_graph.operands().at(index).getDef());
  _graph.operands().at(input_operand_index).insertUse(node_index);
                                            ...
                                            const ir::Layout &to_layout)
{
  ...
  const auto &operand = _graph.operands().at(index);
  if (operand.typeInfo().type() == ir::DataType::FLOAT32 && to_layout == ir::Layout::NHWC)
    return;

  // Update graph operand
  auto output_typeinfo = use_float ? ir::TypeInfo{ir::DataType::FLOAT32} : operand.typeInfo();
  ...
  auto permute_node = std::make_unique<Permute>(output_operand_index, index, permute_type);
  auto node_index = _graph.operations().push(std::move(permute_node));

  // Update use/def info
  _graph.operands().at(output_operand_index).setDef(node_index);
  assert(_graph.operands().at(index).getUses().size() == 0);
  _graph.operands().at(index).insertUse(node_index);
