...
void TrainableGraph::markOpsUnnecessaryForBackward()
{
  ...
  operations().iterate([&](const ir::OperationIndex &op_idx, const ir::IOperation &op) {
    ...
    if (train_op->isTrainable() && op_idx.value() < min_trainable_op_idx.value())
    {
      min_trainable_op_idx = op_idx;
    }
  });
  for (ir::OperationIndex idx{0}; idx.value() < min_trainable_op_idx.value(); idx++)
  {
    auto train_op = dynamic_cast<ir::train::ITrainableOperation *>(&_graph.operations().at(idx));
    assert(train_op);
    train_op->disableBackward();
  }
}
