...
protected:
  ...
  void CreateSimpleModel()
  {
    ...
    _executors->emplace(std::make_unique<InterpExecutor>(*_graph), onert::ir::ModelIndex{0},
                        onert::ir::SubgraphIndex{0});
  }
...
TEST_F(InterpExecutorTest, create_simple)
{
  ...
  ASSERT_NE(_executors->at(onert::exec::ExecutorIndex{0}), nullptr);
}
