...
{
  return first_object.x >= second_object.x && first_object.y >= second_object.y &&
         first_object.z >= second_object.z;
}

size_t AbsDiffInElements(const size_t first_size, const size_t second_size)
{
  return first_size >= second_size ? first_size - second_size : second_size - first_size;
}
...
size_t AbsDiffInElements(const uint2 &first_size, const uint2 &second_size)
{
  ...
  return first_elements_cnt >= second_elements_cnt ? first_elements_cnt - second_elements_cnt
...
size_t AbsDiffInElements(const uint3 &first_size, const uint3 &second_size)
{
  ...
  const size_t second_elements_cnt = second_size.z * second_size.y * second_size.x;
  return first_elements_cnt >= second_elements_cnt ? first_elements_cnt - second_elements_cnt
                                                   : second_elements_cnt - first_elements_cnt;
}
...
CalculatePositionalMaximums(const std::vector<TensorUsageRecord<size_t>> &usage_records)
{
  ...
  for (const auto &task_profile : task_profiles)
  {
    ...
    {
      positional_max[i] = std::max(positional_max[i], task_profile[i].usage_record->tensor_size);
    }
