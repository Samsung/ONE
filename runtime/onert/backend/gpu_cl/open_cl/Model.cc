...
absl::Status GraphFloat32::SetProducer(NodeId producer, ValueId value)
{
  ...
  if (node_ptr == v->producer)
  {
    return absl::AlreadyExistsError(
      absl::StrCat("Node ", producer, " is already a producer of the value ", value));
  }
...
absl::Status GraphFloat32::AddConsumer(NodeId consumer, ValueId value)
{
  ...
  {
    return absl::AlreadyExistsError(
      absl::StrCat("Node ", consumer, " is already a consumer of the value ", value));
  }
...
absl::Status GraphFloat32::ReplaceInput(NodeId node, ValueId old_value, ValueId new_value)
{
  ValueDef *v_old;
...
absl::Status RemovePrecedingNode(GraphFloat32 *graph, const Node *to_remove, const Node *to_keep)
{
  ...
  for (auto output : graph->FindOutputs(to_remove->id))
  {
    ...
    {
      return absl::InvalidArgumentError("Output from to_remove node has other consumers");
    }
  }
...
absl::Status RemoveFollowingNode(GraphFloat32 *graph, const Node *to_remove, const Node *to_keep)
{
...
absl::Status RemoveSimpleNodeKeepInput(GraphFloat32 *graph, const Node *simple_node)
{
  const auto inputs = graph->FindInputs(simple_node->id);
  const auto outputs = graph->FindOutputs(simple_node->id);
  if (inputs.size() != 1 || outputs.size() != 1)
  {
    return absl::FailedPreconditionError("simple_node node must have 1 input and 1 output");
  }
...
absl::Status RemoveSimpleNodeKeepOutput(GraphFloat32 *graph, const Node *simple_node)
{
  const auto inputs = graph->FindInputs(simple_node->id);
  const auto outputs = graph->FindOutputs(simple_node->id);
  if (inputs.size() != 1 || outputs.size() != 1)
  {
    return absl::FailedPreconditionError("simple_node must have 1 input and 1 output");
  }
  const auto input_id = inputs[0]->id;
  const auto output_id = outputs[0]->id;
  const Node *producer = graph->FindProducer(input_id);
  ...
  {
    return absl::FailedPreconditionError("simple_node should be the only consumer on the node.");
  }
...
absl::Status AddOutput(GraphFloat32 *graph, const Node *from_node, Value **output)
{
...
absl::Status ConnectTwoNodes(GraphFloat32 *graph, const Node *from_node, const Node *to_node,
                             Value **output)
{
