...
#include "ops/DynamicUpdateSliceLayer.h"
...
#include "ops/SquaredDifferenceLayer.h"
#include "ops/L2NormalizationLayer.h"
...
void KernelGenerator::visit(const ir::operation::Attention &node)
{
  ...
  const auto layer_idx = node.param().layer_idx;
  ...
  fn->configure(input_tensor, wq_tensor, wk_tensor, wv_tensor, wo_tensor, cos_tensor, sin_tensor,
                mask_tensor, k_cache_tensor, v_cache_tensor, pos_tensor, layer_idx, output_tensor);
