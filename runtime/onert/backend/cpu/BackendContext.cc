...
void BackendContext::planTensors(const std::vector<onert::ir::OpSequenceIndex> &order, const ir::OpSequences &op_seqs, const ir::LowerInfoMap &lower_info)
{
  ...
  // Increasing use count here makes the tensor never be deallocated, i.e it they will be deallocated last.
  ...
  for (const auto op_seq_ind : order)
  {
    ...
    {
      auto op_inputs = graph()->operations().at(op_idx).getInputs() | ir::Remove::DUPLICATED | ir::Remove::UNDEFINED;
      auto op_outputs = graph()->operations().at(op_idx).getOutputs() | ir::Remove::DUPLICATED | ir::Remove::UNDEFINED;
...
ITensorRegistry *BackendContext::tensorGen(const std::vector<onert::ir::OpSequenceIndex> &order, const ir::OpSequences &op_seqs, const ir::LowerInfoMap &lower_info)
{
  auto model_io = (graph()->getInputs() + graph()->getOutputs()) | ir::Remove::UNDEFINED | ir::Remove::DUPLICATED;
...
std::vector<std::pair<ir::OpSequenceIndex, exec::FunctionSequence>> BackendContext::kernelGen()
{
  std::vector<std::pair<ir::OpSequenceIndex, exec::FunctionSequence>> ret;
  return ret;
}
