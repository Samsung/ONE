...
void FullyConnectedLayer::configure(const IPortableTensor *input, const IPortableTensor *weights,
                                    const IPortableTensor *bias, IPortableTensor *deriv_input,
                                    IPortableTensor *output, IPortableTensor *grad_weights,
                                    ...
                                    const std::shared_ptr<train::ExternalContext> &external_context)
{
  ...
  if (weights_format != ir::FullyConnectedWeightsFormat::Default)
    throw std::runtime_error{
      "train FullyConnected: Weight formats other than default are not supported."};

  if (_deriv_input->get_info().shape().rank() > 2)
    throw std::runtime_error{"train FullyConnected: Input ranks greater than 2 are not supported."};
...
void FullyConnectedLayer::backwardFloat32()
{
  ...
  const IPortableTensor *act_deriv_output;
  switch (_activation)
  {
    case ir::Activation::NONE:
      act_deriv_output = _deriv_output;
      break;
    case ir::Activation::RELU:
      ...
      act_deriv_output = _act_deriv_output.get();
      break;
    default:
  ...
  nnfw::cker::FullyConnected(op_params, getShape(act_deriv_output),
                             getBuffer<float>(act_deriv_output), getShape(transposed_weights),
                             getBuffer<float>(transposed_weights), getShape(nullptr), nullptr,
                             getShape(_deriv_input), getBuffer<float>(_deriv_input));

  // Transpose and calculate gradient for weights
  ...
  nnfw::cker::Transpose(transpose_param, getShape(act_deriv_output),
                        getBuffer<float>(act_deriv_output), getShape(transposed_deriv_output),
                        getBuffer<float>(transposed_deriv_output));

  nnfw::cker::FullyConnected(op_params, getShape(transposed_input),
                             getBuffer<float>(transposed_input), getShape(transposed_deriv_output),
                             getBuffer<float>(transposed_deriv_output), getShape(nullptr), nullptr,
  ...
  {
    assert(_grad_bias);
    assert(_grad_bias->total_size() == _deriv_output->total_size());
    memcpy(_grad_bias->buffer(), _deriv_output->buffer(), _grad_bias->total_size());
  }
}
