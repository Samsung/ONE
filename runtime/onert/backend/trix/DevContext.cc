...
DevContext::DevContext() : _dev_handles{}, _model_ids{}, _meta_map{}
{
  ...
  // TODO Change creating thread pool to only create it in parallel batch
  // A thread in this thread pool is assigned for each device
...
ModelID DevContext::registerModel(const std::string &model_file_path)
{
  ...
  // TODO Change to internal trix model context(?) if it is needed
  return model_id;
}
