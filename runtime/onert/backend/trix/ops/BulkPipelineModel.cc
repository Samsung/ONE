...
BulkPipelineModel::BulkPipelineModel(BulkPipelineModel &&other) noexcept
  : _model_path(std::move(other._model_path)), _device_id(other._device_id),
    _ownership(other._ownership), _prepared(other._prepared.load()), _dev(other._dev),
    _model_id(other._model_id), _meta(std::move(other._meta)), _meta_size(other._meta_size),
    _fp(other._fp), _program_buffer(std::move(other._program_buffer)),
    _weight_buffer(std::move(other._weight_buffer)), _next_model(std::move(other._next_model)),
    _buffer_ready(other._buffer_ready.load()),
    _async_fill_future(std::move(other._async_fill_future))
{
  other._dev = 0;
  other._model_id = 0;
  other._meta_size = 0;
  other._fp = nullptr;
  other._prepared = false;
  other._buffer_ready = false;
}

BulkPipelineModel &BulkPipelineModel::operator=(BulkPipelineModel &&other) noexcept
{
  if (this != &other)
  {
    release();

    _model_path = std::move(other._model_path);
    _device_id = other._device_id;
    _ownership = other._ownership;
    _prepared = other._prepared.load();
    _dev = other._dev;
    _model_id = other._model_id;
    _meta = std::move(other._meta);
    _meta_size = other._meta_size;
    _fp = other._fp;
    _program_buffer = std::move(other._program_buffer);
    _weight_buffer = std::move(other._weight_buffer);
    _next_model = std::move(other._next_model);
    _buffer_ready = other._buffer_ready.load();
    _async_fill_future = std::move(other._async_fill_future);
...
bool BulkPipelineModel::prepare()
{
  ...
  try
  {
    ...
    if (_ownership == ModelOwnership::OWNER)
    {
      openDevice();
      allocateBuffers();
      fillBuffers();
                            ...
                            std::vector<IPortableTensor *> &outputs)
{
  if (!_prepared.load())
  {
    throw ModelExecutionException("Model is not prepared: " + _model_path);
  }

  if (!_meta)
  {
    throw ModelExecutionException("Model metadata is not loaded: " + _model_path);
  }
  ...
  if (ret < 0)
  {
    throw ModelExecutionException("runNPU_model() failed for " + _model_path +
...
void BulkPipelineModel::shareBuffersFrom(const BulkPipelineModel &owner)
{
  if (_ownership == ModelOwnership::OWNER)
  {
    throw ModelPreparationException("Cannot share buffers with owner model: " + _model_path);
  }

  if (!owner.isPrepared())
  {
    throw ModelPreparationException("Owner model is not prepared: " + owner.modelPath());
  }

  // Shareing the buffers
...
bool BulkPipelineModel::loadMetadata()
{
  ...
  if (!_fp)
  {
    throw ModelPreparationException("Failed to open model file: " + _model_path);
  }

  _meta = std::make_unique<npubin_meta>();
  if (fread(_meta.get(), NPUBIN_META_SIZE, 1, _fp) != 1)
  {
    throw ModelPreparationException("Failed to read metadata from: " + _model_path);
  }
...
void BulkPipelineModel::allocateBuffers()
{
  if (_ownership != ModelOwnership::OWNER)
  {
    throw ModelPreparationException("Not allowed to allocate buffers for non-owner model: " +
                                    _model_path);
  }

  if (!_meta)
  {
    throw ModelPreparationException("Metadata not loaded for: " + _model_path);
  }

  _program_buffer = std::make_shared<BulkPipelineBuffer>(
    BulkPipelineBuffer::BufferType::PROGRAM, static_cast<size_t>(_meta->program_size), _device_id);
  _weight_buffer = std::make_shared<BulkPipelineBuffer>(
    BulkPipelineBuffer::BufferType::WEIGHT, static_cast<size_t>(_meta->weight_size), _device_id);
...
void BulkPipelineModel::fillBuffers()
{
  ...
  {
    throw ModelPreparationException("Buffers not properly initialized for: " + _model_path);
  }
...
void BulkPipelineModel::registerModel()
{
  ...
  {
    throw ModelPreparationException("Device or buffers not ready for: " + _model_path);
  }
  ...
  if (ret < 0)
  {
    throw ModelPreparationException("Failed to register model: " + _model_path +
                                    ", ret: " + std::to_string(ret));
  }
}
...
void BulkPipelineModel::openDevice()
{
  ...
  if (ret < 0)
  {
    throw ModelPreparationException("Failed to open NPU device for: " + _model_path +
                                    ", ret: " + std::to_string(ret));
  }
}
