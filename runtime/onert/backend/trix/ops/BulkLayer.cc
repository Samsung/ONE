...
void BulkLayer::configure(const std::vector<IPortableTensor *> &input,
                          ...
                          const std::shared_ptr<DevContext> &dev_context)
{
  ...
  model_buf.filepath = binary_path.c_str();
  model_buf.size = _meta->size;

  if (registerNPUmodel(dev_context->getDev(), &model_buf, &_model_id) < 0)
  {
...
void BulkLayer::run()
{
  ...
  {
    throw std::runtime_error("Unable to create NPU request with model id(%d)", _model_id);
  }
  ...
  _dev_context->setDataInfo(&in_info, _inputs);
  _dev_context->setDataInfo(&out_info, _outputs);

  input_buffers input_buf;
  output_buffers output_buf;
  _dev_context->setBuffer(&input_buf, _inputs);
  _dev_context->setBuffer(&output_buf, _outputs);

  if (setNPU_requestData(_dev_context->getDev(), req_id, &input_buf, &in_info, &output_buf,
                         &out_info))
  {
    throw std::runtime_error("Unable to create NPU request for model id(%d)", _model_id);
  }

  if (submitNPU_request(_dev_context->getDev(), req_id))
  {
    throw std::runtime_error("Unable to submit NPU request with req id(%d)", req_id);
  }

  if (removeNPU_request(_dev_context->getDev(), req_id))
  {
    throw std::runtime_error("Unable to remove NPU request with req id(%d)", req_id);
  }
}
