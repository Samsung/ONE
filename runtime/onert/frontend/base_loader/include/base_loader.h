...
void BaseLoader<LoaderDomain, SpecificLoader>::loadSplit(const Operator *op)
{
  ...
  param.rank = _graph.operands().at(input).shape().rank();
...
void BaseLoader<LoaderDomain, SpecificLoader>::loadCast(const Operator *op)
{
  ...
  qasymm8ToUint8(_graph.operands().at(inputs.at(ir::operation::Cast::Input::INPUT)));
  qasymm8ToUint8(_graph.operands().at(outputs.at(0)));
...
void BaseLoader<LoaderDomain, SpecificLoader>::loadOneHot(const Operator *op)
{
  ...
  const auto depth = _graph.operands().at(depth_opidx).template asScalar<int>();
  const auto on_value = _graph.operands().at(on_value_opidx).template asScalar<float>();
  const auto off_value = _graph.operands().at(off_value_opidx).template asScalar<float>();
...
template <typename LoaderDomain, typename SpecificLoader>
void BaseLoader<LoaderDomain, SpecificLoader>::loadAbs(const Operator *op)
{
  ir::OperandIndexSequence inputs;
  ir::OperandIndexSequence outputs;
...
template <typename LoaderDomain, typename SpecificLoader>
void BaseLoader<LoaderDomain, SpecificLoader>::loadSin(const Operator *op)
{
  ir::OperandIndexSequence inputs;
  ir::OperandIndexSequence outputs;
