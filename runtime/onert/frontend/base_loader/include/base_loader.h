...
ir::OperandIndex BaseLoader<LoaderDomain>::loadOperand(const Tensor *tensor, ir::Graph &subg)
{
  ...
  if (src_sparsity != nullptr)
  {
    ...
    bool block16x1_sparsity = dim_metadata_size == 4 && block_rank == 2;
    if (!random_sparsity && !block16x1_sparsity)
      throw std::runtime_error(
          "sparse tensor is supported only for 2D tensor with random or 16x1 block sparsity");
