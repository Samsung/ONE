...
ir::train::LossInfo loadLossInfo(const circle::ModelTraining *circle_model)
{
  ...
  switch (circle_loss)
  {
    case circle::LossFn::LossFn_CATEGORICAL_CROSSENTROPY:
      ir_loss.type = ir::operation::Loss::Type::CATEGORICAL_CROSSENTROPY;
      break;
    case circle::LossFn::LossFn_MEAN_SQUARED_ERROR:
      ir_loss.type = ir::operation::Loss::Type::MEAN_SQUARED_ERROR;
      break;
...
std::unique_ptr<ir::train::TrainingInfo> loadTrainingInfo(std::shared_ptr<const ir::Data> data)
{
  ...
  std::unique_ptr<ir::train::TrainingInfo> ir_model(new ir::train::TrainingInfo);
  {
    ...
    ir_model->setOptimizerInfo(loadOptimizerInfo(circle_model));
    ir_model->setLossInfo(loadLossInfo(circle_model));
