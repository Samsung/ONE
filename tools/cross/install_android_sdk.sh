...
BUILD_TOOLS_PACKAGE_VERSION="30.0.2" # version of build-tools package which will be installed
NDK_VERSION="22.1.7171670" # version of ndk which will be installed
COMMAND_LINE_TOOLS_ARCHIVE="commandlinetools-linux-10406996_latest.zip" # command line tools archive name from site https://developer.android.com/studio/#downloads
NDK_ONLYY=1 # install ndk only (default: false)
...
usage() {
 ...
 printf "  --ndk-only                    - install ndk into install-dir and return, by default: install all sdk\n"
...
check_preconditions() {
  ...
  check_that_available java 'default-jdk or openjdk-{version}-jdk. SDK command line tools require openjdk 16 or higher'
}
...
install_command_line_tools() {
  ...
  cmdline-tools/bin/sdkmanager --sdk_root=$root "cmdline-tools;latest"
  popd > /dev/null
