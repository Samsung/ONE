...
class CicleAdapter(Adapter):
    ...
    def add_map_tensor_to_src(self, tensor_id: int, source_id: int,
                              output_id: int) -> None:
                ...
                graph_builder.MetadataItem(
                    ...
                    attrs=[
                        graph_builder.KeyValue(key='shape',
                                               value=tensor_type + tensor_shape),
                        graph_builder.KeyValue(key='tensor_index', value=f'{tensor_id}'),
                        graph_builder.KeyValue(key='tensor_name',
                                               value=tensor.name.decode('utf-8')),
                    ],
                ))
                ...
                me_node.outputsMetadata.append(
                    graph_builder.MetadataItem(
                        ...
                        attrs=[
                            graph_builder.KeyValue(key='shape',
                                                   value=tensor_type + tensor_shape),
                            graph_builder.KeyValue(key='tensor_index',
                                                   value=f'{tensor_id}'),
                            graph_builder.KeyValue(key='tensor_name',
                                                   value=tensor.name.decode('utf-8')),
                        ]))
        ...
        for idx, op in enumerate(sub_graph.operators):
            ...
            me_node.outputsMetadata.append(
                graph_builder.MetadataItem(
                    ...
                    attrs=[
                        graph_builder.KeyValue(key='shape',
                                               value=output_tensor_type +
                                               output_tensor_shape),
                        graph_builder.KeyValue(key='tensor_index',
                                               value=f'{output_tensor_id}'),
                        graph_builder.KeyValue(key='tensor_name',
                                               value=output_tensor.name.decode('utf-8')),
                    ]))
