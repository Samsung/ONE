 ...
 *
 * @param     [in] g A const reference to an ONNX GraphProto object that contains the graph structure.
 * @param     [in,out] visited A pointer to an integer array used to mark whether nodes have been visited.
 * @pre       The 'visited' array should be pre-allocated with a size at least equal to the number of nodes in the graph.
 ...
 * @param     [in] adjacency_list Adjacency list representing connections between nodes in the graph.
...
* @param     [out] subgraph_node_indices A vector to store indices of nodes included in the subgraph.
        ...
        float max_graph_size)
{
	...
	*subgraph.add_node()=start_node;
	...
	for(int i=0;i<int(adjacency_list[node_index].output_node_index.size());i++)
	{
		int next_node_index=adjacency_list[node_index].output_node_index[i];
		const auto & next_node=g.node(next_node_index);
        if(!visited[next_node_index]&&(depth_out < MAX_DEPTH)&&(graph_size < max_graph_size))        //do deep first search for each successor node 
...
/**
* @brief     Determine and partition subgraphs from the given ONNX graph based on DFS strategy. Compared with determine_subgraphs_v2, this function is more stable but may produce more subgraphs
*
* @param     [in] g The original ONNX graph to be partitioned.
* @param     [out] otherSubgraphs A vector to store the subgraphs that do not meet the preferred operation criteria.
* @param     [in] d The device object containing information about supported and preferred operations.
* @param     [in,out] visited An array to keep track of visited nodes.
* @param     [in] adjacency_list The adjacency list representing the graph's structure.
* @param     [in] strategy The partitioning strategy to be used (e.g., SPILTE_CPU_STRUCTURE_FIRST, SPILTE_NPU_STRUCTURE_FIRST).
*
* @pre       The graph `g` and `adjacency_list` should be properly initialized.
* @pre       The `visited` array should be initialized to zero.
* @pre       The device object `d` should be properly initialized with support and preferred operations.
*
...
/**
* @brief     Determine and partition subgraphs from the given ONNX graph using the index of nodes, compared with determine_subgraphs, this function may produce less subgraphs but some of them may be not fully connected(connectivity of subgrpahs will not affect the inference procedure of subgraphs)
*            This function specifically handles the partitioning logic for NPU support and preferred operations.
*
* @param     [in] g The original ONNX graph to be partitioned.
* @param     [out] otherSubgraphs A vector to store the subgraphs that do not meet the preferred operation criteria.
* @param     [in] d The device object containing information about supported and preferred operations.
...
/**
* @brief     Perform Tarjan's algorithm to find all strongly connected components in a directed graph.
*            This function uses depth-first search (DFS) to identify and group nodes into strongly connected components.
*
* @param     [in] index The current node index being visited.
* @param     [in] depth The current depth in the DFS traversal.
* @param     [out] strongly_connected_subgraphs A vector to store the identified strongly connected components.
...
* @param     [in] successors_Subgraphs A vector of vectors representing the adjacency list of the graph.
*
...
* @pre       The `successors_Subgraphs` should be properly initialized with the graph's adjacency list.
*
* @post      The `strongly_connected_subgraphs` vector will contain all the strongly connected components found in the graph.
* @post      The `DFN` and `LOW` arrays will reflect the discovery times and lowest reachable discovery times for each node.
    ...
    int* LOW, std::vector<int>& stack_subgraphs, std::vector<std::vector<int>>& successors_Subgraphs)
{
    ...
    for(const auto& successor : successors_Subgraphs[index])
    {
        ...
        {
            Tarjan(successor, rank,strongly_connected_subgraphs, DFN, LOW, stack_subgraphs, successors_Subgraphs);//visit successor
    ...
    if(LOW[index] == DFN[index])//if this node is the smallest root of the strongly connected component subtree, then subsequent nodes are popped out of the stack and the obtained strongly connected components are saved.
    {
...
/**
* @brief     Calculate the rank of each node in the merged graph formed by the given strongly connected components.
...
*
* @pre       The `strongly_connected` vector should contain valid indices for `Subgraphs` and `otherSubgraphs`.
* @pre       The `Subgraphs` and `otherSubgraphs` vectors should be properly initialized with ONNX GraphProtos.
*
* @post      The `node_rank_list` vector will contain the nodes of the merged graph with their respective ranks.
*
* @exception None
*
    ...
    )
{
    ...
    while(!finished_flag) 
    {
        ...
        else
        {
            ...
            {
                int find_flag=0;
                if(node_rank_list[i].rank >= 0 && node_rank_list[i].rank < sort_count){continue;}////If it has already been sorted, skip this subgraph
                for(const auto& input : merged_graph.node(i).input())////traveres all inputs of this subgraph
                {
                    for(int j=0; j< merged_graph.node_size(); j++)////examint if the input is the output of j th subgraph
                    {
                        for(const auto& output : merged_graph.node(j).output())
                        {
                            if(output==input)
                            {
                                if((node_rank_list[j].rank < 0 || node_rank_list[j].rank >= sort_count))//the j th subgraph has not been sorted
                                {
                                    find_flag=1;
                                    break;
                                }
...
/**
* @brief     Calculate the rank of each node in the merged graph formed by the given strongly connected components.
*            The rank is determined based on the topological order of the nodes. Compared with calculate_node_rank, this function has different input parameters.
*
* @param     [in] strongly_connected A vector containing indices of strongly connected components.
* @param     [in] Subgraphs A vector of ONNX GraphProtos representing the main subgraphs.
* @param     [in] otherSubgraphs A vector of ONNX GraphProtos representing additional subgraphs.
    ...
    )
{
    ...
    while(!finished_flag) 
    {
        ...
        else
        {
            ...
            {
                int find_flag=0;
                if(node_rank_list[i].rank >= 0 && node_rank_list[i].rank < sort_count){continue;}
                for(const auto& input : merged_graph.node(i).input())////traverses all inputs of this subgraph
                {
                    for(int j=0; j< merged_graph.node_size(); j++)///examint if the input is the output of j th subgraph
                    {
                        for(const auto& output : merged_graph.node(j).output())
...
*            The rank is determined based on the topological order of the nodes.
*            This function is only used to calculate the rank of the nodes in a single graph, especially the original graph
*
* @param     [in] merged_graph The ONNX GraphProto representing the merged graph.
* @param     [out] node_rank_list A vector of `graph_adjacency_node` structures to store the nodes and their ranks.
*
...
/**
* @brief     Determine the cut ranks in the given list of SCC (Strongly Connected Component) node ranks.
*            A cut rank is defined as a rank where no node exists, but there is at least one node at the next rank.
*
* @param     [in] scc_node_rank A vector of `graph_adjacency_node` structures representing the nodes and their ranks.
*
* @pre       The `scc_node_rank` vector should be properly initialized and contain valid node ranks.
*
...
/**
* @brief     Eliminate strongly connected components in the graph and partition them into subgraphs based on node ranks.
*
* @param     [in] strongly_connected_subgraphs List of indices representing strongly connected components.
...
* @post      The `Subgraphs` and `otherSubgraphs` lists may be modified with new partitions based on node ranks.
* @exception None
* @return    None
*/
...
/**
* @brief     Eliminate strongly connected components in the graph and partition them into individual subgraphs.
*
* @param     [in] strongly_connected_subgraphs List of indices representing strongly connected components.
* @param     [in,out] Subgraphs List of subgraphs that will be updated.
* @param     [in,out] otherSubgraphs List of other subgraphs that will be updated.
* @param     [in] g The original graph from which strongly connected components are derived.
* @pre       The input graph `g` should be properly initialized and contain nodes.
* @post      The `Subgraphs` and `otherSubgraphs` lists will be updated with individual nodes from each strongly connected component.
* @exception None
* @return    None
*/
...
* @pre       The `index` should be a valid index within the combined range of `Subgraphs` and `otherSubgraphs`.
* @post      None
* @exception None
...
* @param     [out] scc_pair_cut List of cut subgraphs (upper and lower parts of master graph and slave graph).
...
* @post      None
* @exception None
* @return    Index of the successor or predecessor subgraph with the least number of nodes, or -1 if no such subgraph exists.
*/
int find_min_size(int index, std::vector<int>& successor, std::vector<int>& predecessor,std::vector<onnx::GraphProto> &Subgraphs, std::vector<onnx::GraphProto> &otherSubgraphs)//find the successor or predecessor with the least nodes
{
    ...
    for(int i = 0; i < int(successor.size()); i++)
    {
        ...
        if( (successor[i] < int(Subgraphs.size())&& index <int(Subgraphs.size()))||(successor[i] >= int(Subgraphs.size())&& index >= int(Subgraphs.size())) )
        {
    ...
    for(int i = 0; i < int(predecessor.size()); i++)
    {
        ...
        onnx::GraphProto tempgraph;
        if((predecessor[i] < int(Subgraphs.size())&& index <int(Subgraphs.size()))||(predecessor[i] >= int(Subgraphs.size())&& index >= int(Subgraphs.size())))
        {
...
////othersubgraphs after merged
    ...
    print_subgraphs(Subgraphs, (char*)"./subgraphs_final_2.txt", otherSubgraphs, (char*)"./other_subgraphs_final_2.txt");
    ...
    print_subgraphs(Subgraphs, (char*)"./subgraphs_final_3.txt", otherSubgraphs, (char*)"./other_subgraphs_final_3.txt");
    node_num_all = 0;
    for(const auto& sg : Subgraphs)
    {
        node_num_all += sg.node_size();
    ...
    print_subgraphs(Subgraphs, (char*)"./subgraphs_final_4.txt", otherSubgraphs, (char*)"./other_subgraphs_final_4.txt");
    ////*    
    ...
    while(!finished_flag) 
    {
        ...
        else
        {
            ...
            {
                int find_flag=0;
                if(issort_Subgraphs[i]==1&&i!=int(graphs_inputs.size())-1){continue;}
                for(const auto& g_input : graphs_inputs[i])
                {
                    for(int j=0; j< int(graphs_outputs.size());j++)
                    {
                ...
                if(i==int(graphs_inputs.size())-1)//add the subgraph to the queue only when cycle is completed to prevent the newly added subgraph in this cycle from being the predecessor of the subsequent sub-graph.
                {
                    for(int j=0; j<int(graphs_inputs.size());j++)
                    {
