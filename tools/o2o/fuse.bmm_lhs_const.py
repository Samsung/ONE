...
def count_tensor_usage(subgraph, tensor_index):
    """Count how many operators use a specific tensor as input within a subgraph"""
    count = 0
...
def reshape_fc_weights(model: 'circle.ModelT', subgraph: 'circle.SubGraphT',
    ...
    if len(weights_tensor.shape) > 2 and is_effectively_2d(weights_tensor.shape):
        ...
        usage_count = count_tensor_usage(subgraph, weights_index)
    ...
    for subgraph_idx, subgraph in enumerate(model.subgraphs):
        ...
        for i in range(len(subgraph.operators) - 1, -1, -1):
            ...
            if bmm_op is None or model.operatorCodes[
                    bmm_op.
                    opcodeIndex].builtinCode != circle.BuiltinOperator.BATCH_MATMUL:
                continue
            ...
            final_rhs_tensor_index = add_rhs_transpose_if_needed(
                model, subgraph, bmm_op_idx, rhs_tensor_index, rhs_tensor)
