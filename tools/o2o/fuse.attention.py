...
def find_attention_pattern(model: 'circle.ModelT',
                           subgraph: 'circle.SubGraphT') -> Tuple[int, int, int, int]:
    """
    Dynamically find attention block pattern parameters.
    
    Returns:
        Tuple of (start_offset, block_length, stride, num_blocks)
    ...
    return (first_block_start, block_length, stride, num_blocks)
...
def find_attention_blocks(model: 'circle.ModelT',
    ...
    start_offset, block_length, stride, num_blocks = find_attention_pattern(
        model, subgraph)
