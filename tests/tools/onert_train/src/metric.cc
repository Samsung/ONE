...
template <typename T>
float Metric::categoricalAccuracy(const std::vector<T> &output, const std::vector<T> &expected,
                                  int32_t batch, int32_t size)
{
  int correct = 0;
  for (int b = 0; b < batch; ++b)
  {
    int b_offset = b * size;
    std::vector<T> boutput(output.data() + b_offset, output.data() + b_offset + size);
    std::vector<T> bexpected(expected.data() + b_offset, expected.data() + b_offset + size);
    auto output_idx =
...
float Metric::categoricalAccuracy(int32_t index)
{
  ...
  switch (_infos[index].dtype)
  {
    ...
    {
      std::vector<float> output(static_cast<const float *>(_output[index].data()),
                                static_cast<const float *>(_output[index].data()) + batch * size);
      std::vector<float> expected(static_cast<const float *>(_expected[index].data()),
                                  static_cast<const float *>(_expected[index].data()) +
                                    batch * size);
      return categoricalAccuracy(output, expected, batch, size);
    }
    default:
