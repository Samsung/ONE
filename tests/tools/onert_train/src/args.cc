...
void Args::Initialize(void)
{
  ...
  auto process_trainable_ops = [&](const std::string &trainable_ops_idx_str) {
        ...
        if (range_iter != std::string::npos)
        {
          ...
          if (begin_idx > end_idx)
          {
            std::cerr << "begin_idx=" << begin_idx
                      << " of untrainable operator index cannot be greater than end_idx=" << end_idx
                      << "\n";
            exit(1);
          }
    ...
    {
      std::cerr << "Invalid argument passed as frozen_ops: " << ex.what() << "\n";
      exit(1);
    }
    catch (const std::out_of_range &ex)
    {
      std::cerr << "Out of range argument passed as frozen_ops: " << ex.what() << "\n";
      exit(1);
    }
  };
