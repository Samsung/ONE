...
TEST_F(RawDataLoaderTest, loadDatas_2)
{
  ...
  // Generate test data
  std::vector<float> in(num_input * num_elems(&in_info));
  std::generate(in.begin(), in.end(), [] {
    static float i = 0.f;
    return i += 1.1f;
  });
  std::vector<float> expected(num_expected * num_elems(&expected_info));
  std::generate(expected.begin(), expected.end(), [in] {
    auto sum = std::accumulate(in.begin(), in.end(), 0.f);
    return sum;
  });
  ...
  for (uint32_t i = 0; i < num_sample; ++i)
  {
    ...
    for (uint32_t h = 0; h < num_input; ++h)
    {
      ...
      for (uint32_t j = 0; j < batch_size; ++j)
      {
        for (uint32_t k = 0; k < num_elem; ++k)
        {
          auto inbufs = reinterpret_cast<float *>(inputs[j].data()) + k;
          gen_in[h].emplace_back(*inbufs);
        }
      }
    }
    ...
    for (uint32_t h = 0; h < num_expected; ++h)
    {
      ...
      for (uint32_t j = 0; j < batch_size; ++j)
      {
        for (uint32_t k = 0; k < num_elem; ++k)
        {
          auto exbufs = reinterpret_cast<float *>(expecteds[j].data()) + k;
          gen_ex[h].emplace_back(*exbufs);
        }
      }
    }
