...
// Float32 test case
TEST_F(RawDataLoaderTest, loadDatas_2)
{
  const uint32_t data_length = 100;
  const uint32_t num_input = 1;
  const uint32_t num_expected = 1;
...
// multiple input test case
TEST_F(RawDataLoaderTest, loadDatas_3)
{
  const uint32_t data_length = 100;
  const uint32_t num_input = 2;
  const uint32_t num_expected = 1;
  const uint32_t batch_size = 16;
  ...
  std::vector<nnfw_tensorinfo> in_infos;
  for (uint32_t i = 0; i < num_input; ++i)
  {
    in_infos.emplace_back(in_info);
  }
  ...
  for (uint32_t i = 0; i < num_expected; ++i)
  {
    expected_infos.emplace_back(expected_info);
  }
  ...
  {
    expected[i].resize(num_elems(&expected_infos[i]));
    std::generate(expected[i].begin(), expected[i].end(), [in] {
      auto sum = std::accumulate(in[0].begin(), in[0].end(), 0);
      sum = std::accumulate(in[1].begin(), in[1].end(), sum);
      return sum;
    });
  }
...
// multiple input and output test case
TEST_F(RawDataLoaderTest, loadDatas_4)
{
  const uint32_t data_length = 100;
  const uint32_t num_input = 2;
  const uint32_t num_expected = 2;
  const uint32_t batch_size = 16;
