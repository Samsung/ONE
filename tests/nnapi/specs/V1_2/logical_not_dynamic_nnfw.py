...
Testing LogicalNot op when the input is dynamic.
      input [1, 2, 2, 1]  shape [4]  (value of shape will be [1, 2, 2, 1])
          |             |
          +-------------+
          |
       Reshape (added by DynamicInputGenerator since it generates its output to be dynamic)
         ...
         LogicalNot
          |
        output (dynamic tensor, [1, 2, 2, 1] at execution time)
'''
...
model_output = Output("output", "TENSOR_BOOL8", "{1, 2, 2, 1}")

model.Operation("LOGICAL_NOT", test_node_input).To(model_output)

model_input_data = [False, True, True, True]
