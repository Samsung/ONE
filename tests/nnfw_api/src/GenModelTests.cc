...
{
protected:
  void SetUp() override { ENSURE_NNFW_CALL(nnfw_create_session(&_so.session)); }

  void TearDown() override
  {
    ENSURE_NNFW_CALL(nnfw_load_circle_from_buffer(_so.session, _cbuf.buffer(), _cbuf.size()));
    ENSURE_NNFW_CALL(nnfw_prepare(_so.session));

    // In/Out buffer settings
    {
      uint32_t num_inputs;
      ASSERT_EQ(nnfw_input_size(_so.session, &num_inputs), NNFW_STATUS_NO_ERROR);
      _so.inputs.resize(num_inputs);
      for (uint32_t ind = 0; ind < _so.inputs.size(); ind++)
      {
        nnfw_tensorinfo ti;
        ASSERT_EQ(nnfw_input_tensorinfo(_so.session, ind, &ti), NNFW_STATUS_NO_ERROR);
      ...
      ASSERT_EQ(nnfw_output_size(_so.session, &num_outputs), NNFW_STATUS_NO_ERROR);
      _so.outputs.resize(num_outputs);
      for (uint32_t ind = 0; ind < _so.outputs.size(); ind++)
      {
        nnfw_tensorinfo ti;
        ASSERT_EQ(nnfw_output_tensorinfo(_so.session, ind, &ti), NNFW_STATUS_NO_ERROR);
    ...
    // Set input values, run, and check output values
    {
      ...
      ASSERT_EQ(nnfw_run(_so.session), NNFW_STATUS_NO_ERROR);
    ...
    ASSERT_EQ(nnfw_close_session(_so.session), NNFW_STATUS_NO_ERROR);
  }
