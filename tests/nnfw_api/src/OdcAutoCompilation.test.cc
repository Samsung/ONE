...
const std::string kPathToResources = "./nnfw_api_gtest_models/conv2d/";

const std::string CircleFile = "Conv2D_000.circle";
...
TEST(TestOdcAutoCompilation, AutoCompilation_test)
{
  ...
  nnfw_load_model_from_modelfile(session, (kPathToResources + CircleFile).c_str());
  ...
  std::string compile_model_extension = "abc";

  // Delete previuos quantized and compiled model
  std::string quantized_model_name = kPathToResources +
                                     CircleFile.substr(0, CircleFile.find_last_of(".")) +
                                     std::string(".q.circle");
  std::string compiled_model_name = kPathToResources +
                                    CircleFile.substr(0, CircleFile.find_last_of(".")) +
                                    std::string(".") + compile_model_extension;
...
TEST(TestOdcAutoCompilation, neg_autoCompilation_no_export_path)
{
  ...
  // run model
  NNFW_STATUS status =
    nnfw_run_with_auto_compilation(session, "",
                                   NNFW_CODEGEN_PREF::NNFW_CODEGEN_PREF_DEFAULT);
