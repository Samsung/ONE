...
//   int weight = cgen.addTensor({{1, 2, 2, 4}, circle::TensorType::TensorType_FLOAT32,
//   weight_buf}); int bias = cgen.addTensor({{4}, circle::TensorType::TensorType_FLOAT32,
//   bias_buf}); int out = cgen.addTensor({{1, 2, 1, 4}, circle::TensorType::TensorType_FLOAT32});
...
//   int weight = cgen.addTensor({{1, 3, 1, 2}, circle::TensorType::TensorType_FLOAT32,
//   weight_buf}); int bias = cgen.addTensor({{2}, circle::TensorType::TensorType_FLOAT32,
//   bias_buf}); int out = cgen.addTensor({{1, 2, 2, 2}, circle::TensorType::TensorType_FLOAT32});
...
//   uint32_t bias_buf = cgen.addBuffer(std::vector<float>(2, 0.f));
//   int in = cgen.addTensor({{1, 2, 2, 2}, circle::TensorType::TensorType_FLOAT32});
//   int weight = cgen.addTensor({{1, 3, 3, 2}, circle::TensorType::TensorType_FLOAT32,
//   weight_buf}); int bias = cgen.addTensor({{2}, circle::TensorType::TensorType_FLOAT32,
//   bias_buf}); int out = cgen.addTensor({{1, 2, 2, 2}, circle::TensorType::TensorType_FLOAT32});
//   cgen.addOperatorDepthwiseConv2D({{in, weight, bias}, {out}}, circle::Padding_SAME, 1, 1, 1,
//                                   circle::ActivationFunctionType_NONE);
...
//   int weight = cgen.addTensor({{1, 2, 2, 4}, circle::TensorType::TensorType_FLOAT32,
//   weight_buf}); int bias = cgen.addTensor({{4}, circle::TensorType::TensorType_FLOAT32,
//   bias_buf}); int out = cgen.addTensor({{1, 2, 2, 4}, circle::TensorType::TensorType_FLOAT32});
//   cgen.addOperatorDepthwiseConv2D({{in, weight, bias}, {out}}, circle::Padding_VALID, 1, 1, 2,
...
//   int weight = cgen.addTensor({{1, 2, 2, 4}, circle::TensorType::TensorType_FLOAT32,
//   weight_buf}); int bias = cgen.addTensor({{4}, circle::TensorType::TensorType_FLOAT32,
//   bias_buf}); int out = cgen.addTensor({{1, 2, 1, 4}, circle::TensorType::TensorType_UINT8});
//   cgen.addOperatorDepthwiseConv2D({{in, weight, bias}, {out}}, circle::Padding_VALID, 1, 1, 2,
//                                   circle::ActivationFunctionType_NONE);
//   cgen.setInputsAndOutputs({in}, {out});
