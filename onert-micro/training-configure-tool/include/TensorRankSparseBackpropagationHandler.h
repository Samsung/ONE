...
/*
 * Method to find the most trainable (which gets the best metric result and less peak memory) train ranks
 * for every operation in selected operators indexes.
 * Note: Train rank - this is an indicator of how much data of the current operation we will train
 * (for example, the entire operation, only the bias, only the upper half, and so on)
 */
onert_micro::OMStatus
findBestSparseBackpropagationTensorsRanks(onert_micro::OMConfig &config, TrainData &train_data,
                                          const std::unordered_set<uint16_t> &selected_op_indexes,
                                          std::unordered_map<uint16_t, uint16_t> &best_train_ranks);
