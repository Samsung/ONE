...
OMStatus onert_micro::execute::execute_kernel_CircleCast(const OMExecuteArgs &execute_args)
{
  ...
  const circle::CastOptions *options;
  {
    ...
    options = runtime_kernel.first_operator->builtin_options_as_CastOptions();
  ...
  switch (options->in_data_type())
  {
#ifndef DIS_FLOAT
    case circle::TensorType_BOOL:
    {
      auto input_bool_data = core::utils::castInputData<bool>(input_data);
      switch (options->out_data_type())
      {
        case circle::TensorType_FLOAT32:
        {
          copyCast(input_bool_data, core::utils::castOutputData<float>(output_data), input_shape.flatSize());
          break;
        }
        default:
        {
