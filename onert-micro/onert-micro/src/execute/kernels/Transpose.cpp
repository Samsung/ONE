...
OMStatus onert_micro::execute::execute_kernel_CircleTranspose(const OMExecuteArgs &execute_args)
{
  ...
  const int32_t *perm_data = core::utils::castInputData<int32_t>(input2_data);

  const int32_t size = input2_shape.dims(0);
  ...
  switch (input1->type())
  {
    ...
    {
      pal::Transpose(params, input1_shape, core::utils::castInputData<float>(input1_data),
