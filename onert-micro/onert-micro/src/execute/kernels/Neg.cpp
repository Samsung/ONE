...
template <typename T>
void infer_neg(const T *input_data, T *output_data, size_t flat_size)
{
...
OMStatus onert_micro::execute::execute_kernel_CircleNeg(const OMExecuteArgs &execute_args)
{
  ...
  switch (input->type())
  {
    ...
    {
      infer_neg(core::utils::castInputData<float>(input_data),
                core::utils::castOutputData<float>(output_data), input_shape.flatSize());
    }
    break;
#endif // DIS_FLOAT
    default:
