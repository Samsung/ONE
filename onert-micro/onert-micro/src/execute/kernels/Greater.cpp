...
OMStatus onert_micro::execute::execute_kernel_CircleGreater(const OMExecuteArgs &execute_args)
{
  ...
  core::ComparisonParams op_params;
  ...
  switch (input1->type())
  {
    ...
    case circle::TensorType_FLOAT32:
    {
      ...
      {
        pal::BroadcastComparison4DSlowNoScaling<float>(
          op_params, input1_shape, core::utils::castInputData<float>(input1_data), input2_shape, core::utils::castInputData<float>(input2_data),
          output_shape, core::utils::castOutputData<bool>(output_data), pal::GreaterFn<float>);
      }
      else
      {
        const int64_t flat_size = input1_shape.flatSize();
        pal::ComparisonNoScaling<float>(flat_size, core::utils::castInputData<float>(input1_data), core::utils::castInputData<float>(input2_data),
                                        core::utils::castOutputData<bool>(output_data), pal::GreaterFn<float>);
      }
      break;
    }
#endif // DIS_FLOAT
