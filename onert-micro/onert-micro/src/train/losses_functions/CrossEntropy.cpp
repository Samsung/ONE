 ...
 */
void CrossEntropy::calculateErrorBackpropagation(const uint32_t flat_size, float *calculated_data,
                                                 float *target_data, float *output_grad)
{
  for (uint32_t i = 0; i < flat_size; ++i)
  {
    output_grad[i] = -1.0 * target_data[i] / (calculated_data[i] + float(10.0e-32));
  }
}
