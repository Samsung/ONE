...
void execute_kernel_CirclePadCommon(const circle::Operator *cur_op, BaseRuntimeGraph *runtime_graph)
{
  ...
  auto *input1_data = runtime_graph->getDataByTensor(kernel.input1());
  if (input1_data == nullptr)
    input1_data = runtime_graph->getConstDataByTensor(kernel.input1());
  assert(input1_data);

  auto *input2_data = runtime_graph->getConstDataByTensor(kernel.input2());
  assert(input2_data);
  ...
  switch (Tensor::element_type(kernel.input1()))
  {
    ...
    {
      float pad_value =
        kernel.input3() == nullptr
          ? 0.f
          : *kernels::getTensorData<float>(runtime_graph->getConstDataByTensor(kernel.input3()));
      luci_interpreter_pal::Pad(pad_params, kernels::getTensorShape(kernel.input1()),
                                kernels::getTensorData<float>(input1_data), &pad_value,
                                kernels::getTensorShape(kernel.output()),
