                       ...
                       const circle::Tensor *axis, const circle::Tensor *output, bool keep_dims)
{
  ...
  tflite::reference_ops::ReduceGeneric<T>(
    kernels::getTensorData<T>(tiso_data->input1_data), reinterpret_cast<const int *>(input_dims.data()), input_rank,
    kernels::getTensorData<T>(tiso_data->output_data), reinterpret_cast<const int *>(output_dims.data()), output_rank,
