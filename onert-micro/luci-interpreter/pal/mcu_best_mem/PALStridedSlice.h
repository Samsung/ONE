                         ...
                         luci_interpreter::OperationGraphStatus status)
{
  ...
  // Input scale
  const float input_min_max_range = op_params.input_min_max_range;
  const float input_scale = input_min_max_range / max_int16_value;
  // Output scale
  const float output_min_max_range = op_params.output_min_max_range;
             ...
             !loopCondition(offset_3, end_3, op_params.strides[3]); offset_3 += step_3)
        {
               ...
               offset_4 += op_params.strides[4])
          {
            ...
            if (status == luci_interpreter::OperationGraphStatus::USUAL or
              ...
              input_value = input_data[offset_4];
            else
              input_value = static_cast<T>(int16_input_data[offset_4]) * input_scale;

            if (status == luci_interpreter::OperationGraphStatus::USUAL or
                status == luci_interpreter::OperationGraphStatus::END)
              *output_data++ = input_value;
            else
            {
              *int16_output_data++ = static_cast<int16_t>(std::round(input_value / output_scale));
            }
          }
        }
      }
