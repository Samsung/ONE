...
#include <iostream>
#include <luci/Log.h>
volatile void uart_print(const char *s)
{
  while (*s != '\0')
  {
...
int main()
{
  flatbuffers::Verifier verifier{reinterpret_cast<const uint8_t *>(circle_model_raw),
                                 sizeof(circle_model_raw) / sizeof(circle_model_raw[0])};
  uart_print("circle::VerifyModelBuffer\n");
  if (!circle::VerifyModelBuffer(verifier))
  {
    uart_print("ERROR: Failed to verify circle\n");
  }
  uart_print("OK\n");
  ...
  uart_print("luci::Importer().importModule\n");
  auto module = luci::Importer().importModule(model);
  //  std::cout << "OK\n";
  uart_print("std::make_unique<luci_interpreter::Interpreter>(module.get())\n");
  ...
  uart_print("Fill input tensors with some garbage\n");
  ...
  while (true)
  {
    ...
    uart_print("interpreter->interpret()\n");
