...
# Generate network of Shape-Gather-Unsqueeze-Concat-Expand-Identity with Float32, Rank-3
class net_SgucExpandIdentity(torch.nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def post_process(self, model_path):
        ...
        #
        # modify graph to
        # [input]-Shape-Gather-Unsqueeze-Concat-Expand-Identity-[output]
        ...
        node_id = onnx.helper.make_node('Identity', ['out_Expand'], ['out_Id'],
                                        name='/Id')
        onnx_model.graph.node.insert(102, node_id)
        ...
        info = onnx.helper.make_tensor_value_info('out_Id', onnx.TensorProto.FLOAT,
                                                  [1, 1, 4])
        onnx_model.graph.output.insert(0, info)

        remove_outputs = []
        for output in onnx_model.graph.output:
            if output.name == 'out_Id':
                continue
...
_model_ = net_SgucExpandIdentity()
