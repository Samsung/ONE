...
def compare_outputs(model_name, rtolerance, atolerance, o_name2idx_map, is_nchw=False):
    ...
    for idx in range(len(output_names)):
        ...
        output_np_data = np.fromfile(circle_src_model + '.output' + str(file_idx),
                                     output_np_type)
        circle_src_output_data[output_name] = np.reshape(output_np_data, output_np_shape)

        output_np_data = np.fromfile(circle_two_model + '.output' + str(file_idx),
                                     output_np_type)
