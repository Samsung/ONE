                                      ...
                                      mlir::ConversionPatternRewriter &rewriter) const override
  {
    ...
    else if (group > 1)
    {
      ...
      else
      {
        ...
        if (!splitConv(rewriter, group, intype, pre_tran, filter, bias, outtype, op_name,
...
private:
  ...
  bool splitConv(mlir::ConversionPatternRewriter &rewriter, int64_t group,
                 const mlir::RankedTensorType &intype, mlir::Value &pre_tran, mlir::Value &filter,
                 mlir::Value &bias, const mlir::RankedTensorType &outtype,
                 const std::string &op_name, const std::string &filter_name,
                 const ConvAttrs &convAttrs, mlir::Value &add_op) const
  {
    ...
    // llvm::SmallVector<int32_t, 4> input_perm{0, 2, 3, 1};
    ...
    mlir::Value bias_split_zero;
    {
      ...
      std::vector<float> val;
      for (int32_t c = 0; c < channel_split; ++c)
        val.push_back(0.0f);
      mlir::Location bias0_loc = mlir::NameLoc::get(rewriter.getStringAttr(op_name + "/bias0"));
      bias_split_zero = rewriter.create<ConstOp>(bias0_loc, DenseFPElementsAttr::get(type, val));
    }
    ...
    for (int64_t split = 0; split < group; ++split)
    {
      ...
      auto conv = rewriter.create<Conv2DOp>(
        ...
        convAttrs.dilation_h, convAttrs.dilation_w,
        /*fused_activation_function=*/"NONE",
