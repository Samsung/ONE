...
#ifndef __CIRCLE_MLIR_DIALECT_OPS_SELECT_OP_H__
#define __CIRCLE_MLIR_DIALECT_OPS_SELECT_OP_H__
...
OpFoldResult SelectOp::fold(FoldAdaptor adaptor)
{
  ...
  SmallVector<int64_t, 4> select_values;
  auto num_elements = out_type.getNumElements();
  for (int64_t i = 0; i < num_elements; ++i)
  {
    auto x_val = at_x.at(i);
    auto y_val = at_y.at(i);
    select_values.push_back(at_c.at(i) ? x_val : y_val);
  }
  auto values = ArrayRef<int64_t>(select_values);
...
#endif // __CIRCLE_MLIR_DIALECT_OPS_SELECT_OP_H__
