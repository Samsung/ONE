...
// NOTE this implementation assumes split_dim == 0
// TODO enable split for arbitrary split_dim
template <typename T>
bool splitFPTensor(Value input, int64_t num_splits, RankedTensorType expected_output_type,
                   SmallVectorImpl<OpFoldResult> &results)
{
  ...
  for (int64_t sp = 0; sp < num_splits; ++sp)
  {
    std::vector<T> split_values;
    auto nstart = num_elements * sp;
    auto nend = nstart + num_elements;
    for (auto n = nstart; n < nend; ++n)
    {
      split_values.push_back(input_values[n]);
    }
    auto ele = DenseFPElementsAttr::get(expected_output_type, split_values);
...
LogicalResult SplitOp::fold(FoldAdaptor adaptor, SmallVectorImpl<OpFoldResult> &results)
{
  ...
  if (in_etype.isF32())
  {
    if (!splitFPTensor<float>(input, num_splits, expected_output_type, results))
      return failure();
  }
  else if (in_etype.isF64())
  {
    if (!splitFPTensor<double>(input, num_splits, expected_output_type, results))
      return failure();
  }
  else
    return failure();
