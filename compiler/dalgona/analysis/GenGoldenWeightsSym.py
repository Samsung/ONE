...
# This script tests the values of weights/const of int16 quantized model
# Run dalgona with
...
class GenGoldenWeightsSym(object):
    ...
    def genGoldenWeights(self, fq_path, q_path, tensor_name):
        ...
        if self._granularity == 'layer':
            ...
            w_dfp, w_scale, zp, w_min_ng, w_max_ng = q.float2qint_sym(
                weights, w_min, w_max, bit=16, unsigned=False, narrow_range=True)
            w_fq = q.qint2float_sym(
                w_dfp, w_min, w_max, bit=16, unsigned=False, narrow_range=True)

            # Save quantized weights
            w_quantized = q.float2qint_weights_quant(
                w_fq, w_min_ng, w_scale, dtype=np.int16)
            json_out = json.dumps(
                {
            ...
            w_dfp, w_scale, zp, w_min_ng, w_max_ng = q.float2qint_cw_sym(
                weights,
                w_min,
                w_max,
                bit=16,
    ...
    def genGoldenConst(self, q_path, tensor_name):
        ...
        w_min = np.min(const)
        w_max = np.max(const)
        w_quantized, w_scale, zp, w_min_ng, w_max_ng = q.float2qint_sym(
            const, w_min, w_max, bit=16, unsigned=False, narrow_range=True)
    ...
    def genGoldenPReluAlpha(self, q_path, tensor_name):
        ...
        data = np.asarray(data, dtype=np.int16).reshape(const_shape)
    ...
    def StartAnalysis(self, args):
        ...
        self._dw = False
        #    self._granularity = 'layer'  # Not supported
