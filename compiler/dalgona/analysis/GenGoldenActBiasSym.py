...
# This script tests the values/scale/zero_point of int16 quantized activation/bias
# Run dalgona with
...
class GenGoldenActBiasSym(object):
    ...
    def genGoldenBias(self, q_path, tensor_name, exp_scale, granularity):
        ...
        if granularity == 'layer':
            raise SystemExit("Symmetric quant (int16) only supports CWQ for now")
        elif granularity == 'channel':
            q_data = q.float2qint_bias_sym(data, exp_scale)
            # Save int64 bias
            json_out = json.dumps(
                {
    ...
    def genGoldenActivation(self, minmax_path, q_path, tensor_name):
        ...
        min_ng, max_ng, exp_scale, exp_zero_point = q.nudge_sym(rmin, rmax)
        self._scales[tensor_name] = exp_scale
