...
class MPESRComputer(QErrorComputer):
    def __init__(self, fp32_dir, fq_dir, q_circle_path):
        super().__init__(fp32_dir, fq_dir)
        with open(q_circle_path, 'rb') as f:
            self._model = circle.Model.GetRootAsModel(f.read(), 0)
    ...
    def run(self):
        ...
        for data_idx in range(self._num_data):
            ...
            for fp32_data_path in fp32_results:
                ...
                # Peak Error-to-Scale Ratio (PESR)
                PEAK_ERROR = np.max(diff)
                ...
                for i in range(graph.TensorsLength()):
                    ...
                    if name == tensor_name:
                        assert tensor.Quantization().ScaleLength() == 1
                        SCALE = tensor.Quantization().Scale(0)
                        break

                PESR = PEAK_ERROR / SCALE

                if tensor_name in qerror_map:
                    qerror_map[tensor_name] += PESR
                else:
                    qerror_map[tensor_name] = PESR

                qerror_min = min(PESR, qerror_min)
                qerror_max = max(PESR, qerror_max)
