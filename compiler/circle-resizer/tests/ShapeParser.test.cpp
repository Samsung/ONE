...
class ParseShapeTestFixture : public ::testing::TestWithParam<std::tuple<std::string, std::vector<Shape>>> {};
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Values(
                ...
                // many shapes
                std::make_tuple("[3,4],[5,6]", std::vector<Shape>{Shape{Dim{3}, Dim{4}}, Shape{Dim{5}, Dim{6}}}),
                std::make_tuple("[1],[2]", std::vector<Shape>{Shape{Dim{1}}, Shape{Dim{2}}}),
                std::make_tuple(" [3, 4] , [5,6]", std::vector<Shape>{Shape{Dim{3}, Dim{4}}, Shape{Dim{5}, Dim{6}}}),
                std::make_tuple(" [ 1   ] ,[  2]", std::vector<Shape>{Shape{Dim{1}}, Shape{Dim{2}}}),
                std::make_tuple(" [ 1   ] ,[  2]  ", std::vector<Shape>{Shape{Dim{1}}, Shape{Dim{2}}}),
                std::make_tuple(" [1,2],[3,4,5],[6,7,8,9]", std::vector<Shape>{Shape{Dim{1}, Dim{2}}, Shape{Dim{3}, Dim{4}, Dim{5}}, Shape{Dim{6}, Dim{7}, Dim{8}, Dim{9}}})
                ));
...
INSTANTIATE_TEST_SUITE_P(
        InvalidArgParseShape,
        InvalidArgParseShapeTestFixture,
        ::testing::Values(
                std::string{""},
                std::string{"]"},
                std::string{"1a,2"},
