...
template <loco::DataType DT> std::shared_ptr<Tensor> to_fp32(std::shared_ptr<Tensor> tensor)
{
...
std::shared_ptr<Tensor> fp32(std::shared_ptr<Tensor> tensor)
{
  switch (tensor->dtype())
  {
...
void MAEPrinter::accum_absolute_error(uint32_t output_idx, std::shared_ptr<Tensor> a,
                            ...
                            const std::vector<std::shared_ptr<Tensor>> &second)
{
  ...
  {
    auto first_output = first[output_idx];
    auto second_output = second[output_idx];

    // Cast data to fp32 and then compute absolute error
    auto fp32_first_output = fp32(first_output);
    auto fp32_second_output = fp32(second_output);
