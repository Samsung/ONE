...
def generate_one_direction_LSTM(transformer, X, W, R, B, initial_h, initial_c, P, clip, act, dtype, hidden_size, batch_size):
  """
  ...
  if initial_h is not None:
    ...
    previous_h_state_tensor = transformer.make_constant_tensor(np.zeros((batch_size, hidden_size), dtype=dtype), 'initial_h')[0]

  if initial_c is not None:
    previous_c_state_tensor = initial_c
  else:
    previous_c_state_tensor = transformer.make_constant_tensor(np.zeros((batch_size, hidden_size), dtype=dtype), 'initial_c')[0]

  if P is not None:
    p_tensors = transformer.make_node('Split', P, 3, axis = 0, split=[hidden_size] * 3)
    P = {'i': p_tensors[0], 'o': p_tensors[1], 'f': p_tensors[2]}
  else:
    zero = transformer.make_constant_tensor(np.zeros((hidden_size), dtype=dtype), 'zero_peephole')[0]
    P = {'i': zero, 'o': zero, 'f': zero}

  for i in range(seq_length):
    # it = f(Xt*(Wi^T) + Ht-1*(Ri^T) + Pi (.) Ct-1 + Wbi + Rbi)
    it = transformer.make_node('Gemm', [X[i], W['i'], B['i']], 1, transB=1)
    it = transformer.make_node('Gemm', [previous_h_state_tensor, R['i'], it[0]], 1, transB=1)
    ...
    ft = transformer.make_node('Gemm', [previous_h_state_tensor, R['f'], ft[0]], 1, transB=1)
    ...
    ct = transformer.make_node('Gemm', [previous_h_state_tensor, R['c'], ct[0]], 1, transB=1)
    if clip is not None:
    ...
    ot = transformer.make_node('Gemm', [previous_h_state_tensor, R['o'], ot[0]], 1, transB=1)
...
def transform_unidirectional_LSTM(transformer, original_node, x, tensor_infos, activations, clip, direction, hidden_size, layout):
  inputs = original_node.input
  outputs = original_node.output
  if direction == 'reverse':
    x.reverse()
  ...
  if len(inputs) > 5 and inputs[5] != '':
    direction_dim = layout
    initial_h = transformer.make_node('Squeeze', [inputs[5]], 1, axes=[direction_dim])[0]

  initial_c = None
  if len(inputs) > 6 and inputs[6] != '':
    direction_dim = layout
    initial_c = transformer.make_node('Squeeze', [inputs[6]], 1, axes=[direction_dim])[0]
  ...
  state_h_tensors, state_c_tensor = generate_one_direction_LSTM(transformer, x, w, r, b, initial_h, initial_c, p, clip, act, dtype, hidden_size, batch_size)
  ...
  for h_state in state_h_tensors:
    state_layout_tensors += transformer.make_node("Unsqueeze", [h_state], 1, axes=[seq_length_dim, y_direction_dim])
