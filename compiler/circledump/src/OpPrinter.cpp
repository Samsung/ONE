...
public:
  void options(const circle::Operator *op, std::ostream &os) const override {
    if (auto cast_params = op->builtin_options_as_CastOptions()) {
      os << "    ";
      os << "in_data_type("
         << circle::EnumNameTensorType(cast_params->in_data_type()) << ") ";
      os << "out_data_type("
         << circle::EnumNameTensorType(cast_params->out_data_type()) << ") ";
      os << std::endl;
    }
  }
};
...
public:
  void options(const circle::Operator *op, std::ostream &os) const override {
    ...
    // circle_convert.
    // When the converted circle file (with custom op) is opened with hexa
    // editory,
