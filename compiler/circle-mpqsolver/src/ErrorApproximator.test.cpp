...
TEST(CircleMPQSolverErrorApproximatorTest, verifyResultsTest)
{
  ...
  auto value = mpqsolver::ErrorApproximator::approximate(g._conv);
  float expected = ((g._w_max - g._w_min) * g._channel_size * std::max(g._a_max, g._a_min) +
                   ...
                   g._f_h * g._f_w * g._height * g._width * g._channel_size;
  EXPECT_TRUE(::fabs(value * 1.e+06f - expected) < 1.e-04);
}
...
TEST(CircleMPQSolverErrorApproximatorTest, verifyResultsTest_NEG)
{
  ...
  auto value = mpqsolver::ErrorApproximator::approximate(g._input);
  EXPECT_TRUE(::fabs(value - 0.f) < 1.e-04);

  value = mpqsolver::ErrorApproximator::approximate(g._output);
  EXPECT_TRUE(::fabs(value - 0.f) < 1.e-04);
}
