...
TEST(CircleMPQSolverDepthParameteriserTest, verifyResultsTest)
{
  ...
  mpqsolver::NodeDepthType nodes_depth;
  float min_depth = std::numeric_limits<float>().max();  // should be  zero
  float max_depth = -std::numeric_limits<float>().max(); // should be 2 (at output)
  auto status =
    mpqsolver::DepthParameterizer::compute_depth(m.get(), nodes_depth, min_depth, max_depth);

  EXPECT_TRUE(status == EXIT_SUCCESS);
