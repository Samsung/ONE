...
TEST(CircleMPQSolverQuantizerTest, verifyResultsTest)
{
  ...
  mpqsolver::LayerParams params;
  auto res = mpqsolver::Quantizer::quantize(m.get(), "uint8", params);
  EXPECT_TRUE(res);
  auto quant_param = add->quantparam();
  EXPECT_TRUE(quant_param != nullptr);
  EXPECT_TRUE(quant_param->scale.size() == 1);
  EXPECT_TRUE(::fabs(quant_param->scale[0] - range / 255.f) < 1.e-06);
...
{
  mpqsolver::LayerParams params;
  auto res = mpqsolver::Quantizer::quantize(nullptr, "uint8", params);
  EXPECT_TRUE(!res);
}
