...
class NodeGraphlet : public NodeGraphletT<luci::CircleSign>
{
public:
...
TEST(ConnectNodeTest, connect_Sign)
{
  TestNodeGraph tng;
  tng.init({2, 3});
  ...
  auto *node = tng.node();
  ASSERT_NO_THROW(loco::must_cast<luci::CircleSign *>(node));

  auto *clone = luci::clone_node(node, cth.graph_clone());
  ASSERT_NO_THROW(loco::must_cast<luci::CircleSign *>(clone));
...
TEST(ConnectNodeTest, connect_Sign_NEG)
{
  TestNodeGraph tng;
  tng.init({2, 3});
