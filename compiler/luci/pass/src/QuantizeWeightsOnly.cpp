                                        ...
                                        std::vector<float> &nudged_max, int32_t &channel_dim_index)
{
  ...
  const auto kSearchIterations = 12;
  ...
  // accumulators to compare overall MSE
  // total_mse contains sum of diff*diff for all of corresponding weights
  static long long total_number_of_values = 0;
  static double total_mse_opt = 0;
  static double total_mse_base = 0;

  total_number_of_values += min.size();
  ...
  {
    total_mse_base += channel_mse_base[i];
    ...
    total_mse_opt += channel_mse_opt[i];
  }
  iterate_per_channel(node, channel_dim_index, quantize);

  LOGGER(l);
  WARN(l) << "Base: " << std::sqrt(total_mse_base / total_number_of_values)
          << " Opt: " << std::sqrt(total_mse_opt / total_number_of_values);
