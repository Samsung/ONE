...
/**
 *  As transpose1 only changed attribute/input instead of creating new Transpose op,
 *  relu can be trversed first before shape of transpose1 is updated.
 *  If then shape of transpose2 becomes 3x5x5x1 and it causes an error at conv2d.
 *
 * <Initial graph>
 *                                                4x1x1x3
 * [old_perm] ----------+              [filter] ----------+
...
TEST(CircleShapeInferencePassTest, original_node_change)
{
  ...
  old_perm->at<loco::DataType::S32>(1) = 1;
  old_perm->at<loco::DataType::S32>(2) = 2;
