...
public:
  ...
  luci::FusedActFunc _act_function = luci::FusedActFunc::NONE;
...
bool GRUPattern1::matched()
{
  ...
  _act_function = luci::FusedActFunc::NONE;
  _time_major = false;
...
luci::CircleGRU *FuseGRU::create_circle_gru(loco::Graph *graph)
{
  ...
  circle_gru->fusedActivationFunction(_p->_act_function);
  ...
  circle_gru->dtype(loco::DataType::FLOAT32);
...
void FuseGRU::apply()
{
  ...
  auto circle_gru = create_circle_gru(graph);
  ...
  luci::add_origin(circle_gru, luci::composite_origin(origin_vec));

  replace(_p->_pattern_last_node).with(circle_gru);
}
