...
namespace
{
bool check_type_and_shape_equality(luci::CircleNode *left, luci::CircleNode *right)
{
...
template <loco::DataType D> void sum_const_values(luci::CircleConst *left, luci::CircleConst *right)
{
...
bool fuse_horizontal_fc_nodes(luci::CircleAdd *add_node)
{
  ...
  auto left_input_node = dynamic_cast<luci::CircleNode *>(left_fc_node->input());
  if (left_input_node == nullptr)
    return false;

  auto right_input_node = dynamic_cast<luci::CircleNode *>(right_fc_node->input());
  if (right_input_node == nullptr)
    return false;

  if (left_input_node != right_input_node)
    return false;
  ...
  switch (left_fc_weights->dtype())
  {
    ...
    case loco::DataType::S32:
      sum_const_values<loco::DataType::S32>(fused_fc_weights, right_fc_weights);
    default:
      return false;
  }
  ...
  if (fused_fc_bias != nullptr)
  {
    switch (left_fc_bias->dtype())
    {
      case loco::DataType::FLOAT32:
        ...
        break;
      case loco::DataType::S32:
        sum_const_values<loco::DataType::S32>(fused_fc_bias, right_fc_bias);
      default:
        return false;
    }
  }
  ...
  fused_fc_node->input(left_input_node);
