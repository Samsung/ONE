...
bool is_non_negative(luci::CircleConst *node)
{
...
bool is_batchnorm_mul(luci::CircleMul *mul, luci::CircleNode *&pred_node, luci::CircleConst *&gamma)
{
  auto x = static_cast<luci::CircleNode *>(mul->x());
  auto y = static_cast<luci::CircleNode *>(mul->y());

  luci::CircleNode *pred;
  ...
  {
    pred = y;
    constant = static_cast<luci::CircleConst *>(x);
  }
  else if (x->opcode() != luci::CircleOpcode::CIRCLECONST &&
           y->opcode() == luci::CircleOpcode::CIRCLECONST)
  {
    pred = x;
    constant = static_cast<luci::CircleConst *>(y);
  }
  else
  {
    return false;
...
bool is_batchnorm_add(luci::CircleAdd *add, luci::CircleMul *&mul, luci::CircleConst *&beta)
{
  auto x = static_cast<luci::CircleNode *>(add->x());
  auto y = static_cast<luci::CircleNode *>(add->y());
...
luci::CircleConv2D *get_forward_conv2d(luci::CircleNode *node, uint32_t channel_size)
{
  ...
  {
    auto mean = static_cast<luci::CircleMean *>(node);
    ...
    auto succ_node = static_cast<luci::CircleNode *>(*succ.begin());
...
luci::CircleConv2D *get_backward_conv2d(luci::CircleNode *node, uint32_t channel_size)
{
           ...
           opcode == luci::CircleOpcode::ADD)
  {
    ...
    for (auto pred : preds)
    {
      auto pred_conv = get_backward_conv2d(static_cast<luci::CircleNode *>(pred), channel_size);
...
bool update_conv_bias_with_beta(luci::CircleConv2D *conv, luci::CircleConst *beta, bool add_beta)
{
  assert(beta->rank() == 1);
...
void update_conv_weights_with_gamma(luci::CircleConv2D *conv, luci::CircleConst *gamma)
{
  auto filter = dynamic_cast<luci::CircleConst *>(conv->filter());
...
bool fuse_sub_with_conv(luci::CircleSub *sub)
{
  ...
  add = get_forward_add(static_cast<luci::CircleNode *>(*succs.begin()));
  ...
  if (x->opcode() == luci::CircleOpcode::CONV_2D)
  {
    conv = dynamic_cast<luci::CircleConv2D *>(x);
  }
  else if (y->opcode() == luci::CircleOpcode::CONV_2D)
  {
    conv = dynamic_cast<luci::CircleConv2D *>(y);
  }
  else
  {
    return false;
...
bool fuse_add_with_conv(luci::CircleAdd *add, std::vector<luci::CircleSub *> &sub_list)
{
  ...
  {
    pred = y;
    beta = static_cast<luci::CircleConst *>(x);
  }
  else if (x->opcode() != luci::CircleOpcode::CIRCLECONST &&
           y->opcode() == luci::CircleOpcode::CIRCLECONST)
  {
    pred = x;
    beta = static_cast<luci::CircleConst *>(y);
  }
  else
  {
    return false;
  ...
  else if (pred->opcode() == luci::CircleOpcode::ADD)
  {
    ...
    {
      conv = get_backward_conv2d(static_cast<luci::CircleNode *>(pred_of_pred), channel_size);
      if (conv != nullptr)
        break;
    }
    ...
    auto relu_node = static_cast<luci::CircleRelu *>(relu);
...
bool fuse_mul_with_conv(luci::CircleMul *mul)
{
  ...
  {
    auto conv = get_forward_conv2d(static_cast<luci::CircleNode *>(s), channel_size);
    if (conv == nullptr)
      fusable = false;
  }
