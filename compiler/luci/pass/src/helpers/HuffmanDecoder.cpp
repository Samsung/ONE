...
template <typename T>
std::shared_ptr<Node<T>> HuffmanDecoder<T>::importHuffmanTreeFromBoolVec(std::vector<bool> &vec,
                                                                         size_t &index)
{
  if (vec.empty())
    return nullptr;
...
template <typename T>
typename HuffmanDecoder<T>::EncodedTreeAndData
HuffmanDecoder<T>::unpackArrayToEncodedTreeAndData(const uint8_t *pack_ptr)
{
...
template <typename T>
typename HuffmanDecoder<T>::EncodedTreeAndData
HuffmanDecoder<T>::unpackArrayToEncodedTreeAndData(const std::vector<uint8_t> &packed_vec)
{
  constexpr auto kTreeSizeBytesN = sizeof(size_t);
  constexpr auto kDataSizeBytesN = sizeof(size_t);
...
template <typename T>
void HuffmanDecoder<T>::decode(std::shared_ptr<Node<T>> node, std::vector<bool> &vec,
                               uint8_t *dst_ptr)
{
  if (node == nullptr)
  {
...
template <typename T> void HuffmanDecoder<T>::init_decoder(const uint8_t *input)
{
  size_t index = 0;
...
template <typename T> size_t HuffmanDecoder<T>::decode_n(uint8_t *dst_ptr, size_t num)
{
...
template <typename T> std::vector<T> HuffmanDecoder<T>::decode(const std::vector<uint8_t> &input)
{
...
template class HuffmanDecoder<int8_t>;
