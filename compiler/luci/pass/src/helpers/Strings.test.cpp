...
TEST(StringsTest, parse_comma_separated_str)
{
  std::string input1 = "uint8,int16,float32";
  auto result1 = luci::parse_comma_separated_str(input1);
  ASSERT_EQ(3, result1.size());
  ASSERT_EQ("uint8", result1[0]);
  ASSERT_EQ("int16", result1[1]);
  ASSERT_EQ("float32", result1[2]);

  std::string input2 = "uint8,int16,float32,";
  auto result2 = luci::parse_comma_separated_str(input2);
  ASSERT_EQ(4, result2.size());
  ASSERT_EQ("uint8", result2[0]);
  ASSERT_EQ("int16", result2[1]);
  ASSERT_EQ("float32", result2[2]);
  ASSERT_EQ("", result2[3]);

  std::string input3 = "uint8";
  auto result3 = luci::parse_comma_separated_str(input3);
  ASSERT_EQ(1, result3.size());
  ASSERT_EQ("uint8", result3[0]);
}
