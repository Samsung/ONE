...
TEST(LayerInfoMapTest, multiple_output_node_test)
{
  ...
  EXPECT_EQ("split0", map["split"]->name);
  EXPECT_EQ("split0", map["split0"]->name);
  EXPECT_EQ("split0", map["split1"]->name);

  EXPECT_EQ(loco::DataType::U8, map["split"]->dtype);
  EXPECT_EQ(luci::QuantizationGranularity::ChannelWise, map["split"]->granularity);
}
