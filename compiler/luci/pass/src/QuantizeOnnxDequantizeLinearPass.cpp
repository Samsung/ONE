...
public:
  void apply(void)
  {
    ...
    {
      // Onnx does not support int4 as of writing. We assume uint8
      // tensor is actually int4 or uint4 if values are within [0,15]
...
private:
  luci::CircleConst *gen_s4_quant(void)
  {
    ...
    {
      uint8_t u8_val = _p.input->at<loco::DataType::U8>(i);
  ...
  luci::CircleConst *gen_u8_quant(void)
  {
    ...
    {
      uint8_t u8_val = _p.input->at<loco::DataType::U8>(i);
      quantized_node->at<loco::DataType::U8>(u8_val);
    }
...
private:
  luci::CircleConst *gen_s16_quant(void)
  {
    ...
    {
      uint8_t u8_val = _p.input->at<loco::DataType::U8>(i);
      quantized_node->at<loco::DataType::S16>(static_cast<int16_t>(u8_val));
    }
