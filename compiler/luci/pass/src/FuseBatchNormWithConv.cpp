...
#include "luci/Pass/FuseBatchNormWithConv.h"
...
bool fused_batch_norm_with_conv(luci::CircleAdd *add)
{
    ...
    for (uint32_t h = 0; h < filter_height; h++)
    {
      for (uint32_t w = 0; w < filter_width; w++)
      {
        for (uint32_t b = 0; b < filter_in_channel; b++)
        {
          ...
          fused_filter->at<loco::DataType::FLOAT32>(offset) = filter->at<loco::DataType::FLOAT32>(offset) * scale->at<loco::DataType::FLOAT32>(c);
        }
      }
    }
  }
  ...
  {
    fused_bias->at<loco::DataType::FLOAT32>(b) = bias->at<loco::DataType::FLOAT32>(b) * scale->at<loco::DataType::FLOAT32>(b) + shift->at<loco::DataType::FLOAT32>(b);
  }
