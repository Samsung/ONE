...
#define CHECK_OR_FALSE(condition) \
  ...
  uint32_t ifm_channel_depth = ifm_circle->dim(3).value(); // TODO Which value should be selected for unknown?
...
void fuse_instance_norm(const InstanceNormPattern &p)
{
  ...
  auto reshape_beta = graph->nodes()->create<luci::CircleReshape>();
  {
    ...
    uint32_t ifm_channel_depth = circle_ifm->dim(3).value(); // TODO Which value should be selected for unknown?
