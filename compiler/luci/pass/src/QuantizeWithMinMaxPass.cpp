...
void asym_wquant_with_minmax(CircleConst *node, float min, float scaling_factor)
{
  const int32_t kMinScale = 0;
  const int32_t kMaxScale = 255;
  ...
  bool visit(luci::CircleNode *node)
  {
    ...
    for (uint32_t i = 0; i < arity; i++)
    {
      ...
      if (has_min_max(circle_node) && !is_weights(circle_node))
      {
        ...
        circle_node->quantparam()->max[0] = nudged_min;
        circle_node->quantparam()->min[0] = nudged_max;
...
{
  QuantizeBias(loco::DataType input, loco::DataType output, QuantizationGranularity granularity)
...
{
  QuantizeWeights(loco::DataType input, loco::DataType output, QuantizationGranularity granularity)
      : input_type(input), output_type(output), granularity(granularity)
  {
  }
    ...
    for (uint32_t i = 0; i < arity; i++)
    {
      ...
      if (is_weights(circle_node))
      {
        ...
        else
        {
          ...
          asym_wquant_with_minmax(circle_const, min, scaling_factor);
        }
      }
    }
    return false;
