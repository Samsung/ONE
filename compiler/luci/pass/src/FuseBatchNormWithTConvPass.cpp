...
bool fused_batch_norm_with_tconv(luci::CircleAdd *add)
{
  ...
  // allow scale/shift and bias shape of [N], [1,N], [N,1], [1,1,N], ...
  for (uint32_t d = 0; d < scale->rank(); ++d)
  {
    if (filter_out_chn != scale->dim(d).value() && 1 != scale->dim(d).value())
      return false;
  }
  for (uint32_t d = 0; d < shift->rank(); ++d)
  {
    if (filter_out_chn != shift->dim(d).value() && 1 != shift->dim(d).value())
      return false;
  }
  if (bias)
  {
    if (bias->dtype() != loco::DataType::FLOAT32)
      return false;
    for (uint32_t d = 0; d < bias->rank(); ++d)
    {
      if (filter_out_chn != bias->dim(d).value() && 1 != bias->dim(d).value())
        return false;
    }
  }
