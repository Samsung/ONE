...
bool ForceQuantParamPass::run(loco::Graph *g)
{
  ...
  for (auto node : loco::active_nodes(loco::output_nodes(g)))
  {
    ...
    auto target = std::find(tensors.begin(), tensors.end(), name);
    if (target == tensors.end())
      continue;

    auto index = target - tensors.begin();
    auto scale = scales[index];
    auto zp = zerops[index];
    set_qparam(cnode, scale, zp);

    tensors.erase(tensors.begin() + index);
    scales.erase(scales.begin() + index);
    zerops.erase(zerops.begin() + index);
  }
