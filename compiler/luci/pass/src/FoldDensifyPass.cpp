...
{
  if (const_node->sparsityparam() == nullptr)
  {
    throw std::runtime_error("Given constant node has no sparsity parameter");
  }
  ...
  else
  {
    ...
    for (size_t i = 0; i < const_items; ++i)
    {
      ...
      float vv = fp16::fp16_ieee_to_fp32_value(f_data[i]);
      printf("sparse %lu %f\r\n", i, vv);
    }
    ...
    for (uint32_t i = 0; i < dim_size; ++i)
    {
      ...
      float vv = fp16::fp16_ieee_to_fp32_value(data_dense[i]);
      printf("dense %u %f\r\n", i, vv);
    }
