...
void quant_bias(CircleConst *node, float input_scale, float weight_scale, float *scaling_factor,
                int64_t *zp)
{
  ...
  for (int i = 0; i < static_cast<int32_t>(size); ++i)
  {
    node->at<loco::DataType::S32>(i) = std::min(kScale, std::max(-kScale, quantized_values[i]));
  }
  *scaling_factor = scale;
  *zp = 0;
}
...
/**
 * @brief QuantizeActivationWeights quantizes tensors for activations and weights
 * @details For activations, quantize using recorded min/max values
 *          For weights and bias (CircleConst), find min/max values on the fly and then quantize
 */
struct QuantizeActivationWeights final : public luci::CircleNodeMutableVisitor<bool>
{
  QuantizeActivationWeights(loco::DataType input, loco::DataType output)
  ...
  {
    std::cout << "QuantizeActivationWeights visit node: " << node->name() << std::endl;
    ...
    for (uint32_t i = 0; i < arity; i++)
    {
      ...
      else
      {
        ...
        {
          throw oops::UserExn("Min/Max values of all non-const nodes must have been recorded");
        }
      }
    }
    return false;
...
struct QuantizeBias final : public luci::CircleNodeMutableVisitor<bool>
{
  ...
  bool visit(luci::CircleNode *node)
  {
    ...
    float scaling_factor;
    int64_t zp;
    quant_bias(circle_const, input_scale, weight_scale, &scaling_factor, &zp);
    auto quantparam = std::make_unique<CircleQuantParam>();
    quantparam->scale.push_back(scaling_factor);
    quantparam->zerop.push_back(zp);
    assert(circle_const->quantparam() == nullptr); // bias should not be quantized yet
...
bool QuantizeWithMinMaxPass::run(loco::Graph *g)
{
  ...
  // Quantize activation and weights
  for (auto node : loco::active_nodes(loco::output_nodes(g)))
  {
    QuantizeActivationWeights qaw(_input_dtype, _output_dtype);
    auto circle_node = loco::must_cast<luci::CircleNode *>(node);
    circle_node->accept(&qaw);
  }
