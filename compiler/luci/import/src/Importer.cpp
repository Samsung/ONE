...
#include "luci/Import/Nodes/CircleConstReference.h"
                   ...
                   loco::Graph *graph, bool copy_consts)
{
  ...
  for (uint32_t i = 0; i < tensors.size(); ++i)
  {
    ...
    // create copy of const data if data is 'String' or copying requested by caller
    if (copy_consts || luci::luci_datatype(tensors[i]->type()) == loco::DataType::STRING)
      const_node = luci::create_circleconst(&gb_context, i);
    else
      const_node = luci::create_circle_const_ref(&gb_context, i);
...
std::unique_ptr<loco::Graph> Importer::import(const circle::Model *model) const
{
  ...
  // not copy constants if model's buffer provided
  convert_graph(*source_ptr, reader, graph.get(), _model_buffer == nullptr);
