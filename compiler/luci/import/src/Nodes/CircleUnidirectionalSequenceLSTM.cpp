    ...
    const circle::OperatorT &op, const std::vector<CircleNode *> &inputs, loco::Graph *graph) const
{
  ...
  if (auto input_to_input_weights = dynamic_cast<luci::CircleOutputExclude *>(node->input_to_input_weights()))
  {
  ...
  if (auto recurrent_to_input_weights = dynamic_cast<luci::CircleOutputExclude *>(node->recurrent_to_input_weights()))
  {
  ...
  if (auto cell_to_input_weights = dynamic_cast<luci::CircleOutputExclude *>(node->cell_to_input_weights()))
  {
    cell_to_input_weights->dtype(loco::DataType::FLOAT32);
  }
  node->cell_to_forget_weights(inputs.at(10)); // Optional
  if (auto cell_to_forget_weights = dynamic_cast<luci::CircleOutputExclude *>(node->cell_to_forget_weights()))
  {
    cell_to_forget_weights->dtype(loco::DataType::FLOAT32);
  }
  node->cell_to_output_weights(inputs.at(11)); // Optional
  if (auto cell_to_output_weights = dynamic_cast<luci::CircleOutputExclude *>(node->cell_to_output_weights()))
  {
  ...
  if (auto projection_weights = dynamic_cast<luci::CircleOutputExclude *>(node->projection_weights()))
  {
  ...
  if (auto input_layer_norm_coefficients = dynamic_cast<luci::CircleOutputExclude *>(node->input_layer_norm_coefficients()))
  {
    input_layer_norm_coefficients->dtype(loco::DataType::FLOAT32);
  }
  node->forget_layer_norm_coefficients(inputs.at(21)); // Optional
  if (auto forget_layer_norm_coefficients = dynamic_cast<luci::CircleOutputExclude *>(node->forget_layer_norm_coefficients()))
  {
    forget_layer_norm_coefficients->dtype(loco::DataType::FLOAT32);
  }
  node->cell_layer_norm_coefficients(inputs.at(22));   // Optional
  if (auto cell_layer_norm_coefficients = dynamic_cast<luci::CircleOutputExclude *>(node->cell_layer_norm_coefficients()))
  {
    cell_layer_norm_coefficients->dtype(loco::DataType::FLOAT32);
  }
  node->output_layer_norm_coefficients(inputs.at(23)); // Optional
  if (auto output_layer_norm_coefficients = dynamic_cast<luci::CircleOutputExclude *>(node->output_layer_norm_coefficients()))
  {
