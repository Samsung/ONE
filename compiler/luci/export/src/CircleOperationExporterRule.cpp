...
public:
  ...
  std::vector<int32_t> visit(luci::CircleBidirectionalSequenceLSTM *node) final
  {
    ...
    std::vector<int32_t> outputs_vec(bidi_lstm_outs.size()); // WARN : Current code seems wrong?
...
void OperationExporterRule::visit(luci::CircleNode *node)
{
  ...
  auto op_idx = _ctx.md.registerBuiltinOpcode(circle_builtin_operator(node), node->op_version(),
                                              circle_custom_code(node));
