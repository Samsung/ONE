...
#ifndef __LUCI_IR_CIRCLE_CONV_2D_INPUT_GRAD_H__
#define __LUCI_IR_CIRCLE_CONV_2D_INPUT_GRAD_H__
...
/**
 * @brief CONV_2D_INPUT_GRAD in Circle
 */
class CircleConv2DInputGrad final : public FixedArityNode<2, CircleNodeImpl<CircleOpcode::CONV_2D_INPUT_GRAD>>
  {
  public:
    loco::Node *input_grad(void) const { return at(0)->node(); }
    void input_grad(loco::Node *node) { at(0)->node(node); }

    loco::Node *weight(void) const { return at(1)->node(); }
    void weight(loco::Node *node) { at(1)->node(node); }
...
#endif // __LUCI_IR_CIRCLE_CONV_2D_INPUT_GRAD_H__
