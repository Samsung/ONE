...
public:
  void options(const tflite::Operator *op, std::ostream &os) const override {
    if (auto *params = op->builtin_options_as_AddOptions()) {
      os << "    ";
      os << "Activation("
         << EnumNameActivationFunctionType(params->fused_activation_function())
         << ") ";
      os << std::endl;
    }
  }
...
public:
  void options(const tflite::Operator *op, std::ostream &os) const override {
    if (auto cast_params = op->builtin_options_as_CastOptions()) {
      os << "    ";
      os << "in_data_type("
         << tflite::EnumNameTensorType(cast_params->in_data_type()) << ") ";
      os << "out_data_type("
         << tflite::EnumNameTensorType(cast_params->out_data_type()) << ") ";
      os << std::endl;
    }
  }
};
...
public:
  void options(const tflite::Operator *op, std::ostream &os) const override {
    if (auto conv_params = op->builtin_options_as_Conv2DOptions()) {
      ...
      os << "Activation(" << EnumNameActivationFunctionType(
...
public:
  void options(const tflite::Operator *op, std::ostream &os) const override {
    if (auto pool_params = op->builtin_options_as_Pool2DOptions()) {
      ...
      os << "Activation(" << EnumNameActivationFunctionType(
                                 pool_params->fused_activation_function())
         << ")";
      os << std::endl;
    }
  }
...
public:
  void options(const tflite::Operator *op, std::ostream &os) const override {
    if (auto *params = op->builtin_options_as_FullyConnectedOptions()) {
      os << "    ";
      os << "WeightFormat("
         << EnumNameFullyConnectedOptionsWeightsFormat(params->weights_format())
         << ") ";
      os << "Activation("
         << EnumNameActivationFunctionType(params->fused_activation_function())
         << ") ";
...
public:
  void options(const tflite::Operator *op, std::ostream &os) const override {
    ...
    auto attr_map =
        flexbuffers::GetRoot(option_buf->data(), option_buf->size()).AsMap();
    ...
    // Note: attr in "Shape" type does not seem to be converted by
    // tflite_convert.
    // When the converted tflite file (with custom op) is opened with hexa
    // editory,
...
OpPrinterRegistry::OpPrinterRegistry() {
  ...
  _op_map[tflite::BuiltinOperator_AVERAGE_POOL_2D] =
      make_unique<Pool2DPrinter>();
  _op_map[tflite::BuiltinOperator_CAST] = make_unique<CastPrinter>();
  _op_map[tflite::BuiltinOperator_CONCATENATION] =
      make_unique<ConcatenationPrinter>();
  _op_map[tflite::BuiltinOperator_CONV_2D] = make_unique<Conv2DPrinter>();
  _op_map[tflite::BuiltinOperator_DEPTHWISE_CONV_2D] =
      make_unique<DepthwiseConv2DPrinter>();
  _op_map[tflite::BuiltinOperator_DIV] = make_unique<DivPrinter>();
  _op_map[tflite::BuiltinOperator_FULLY_CONNECTED] =
      make_unique<FullyConnectedPrinter>();
  ...
  _op_map[tflite::BuiltinOperator_STRIDED_SLICE] =
      make_unique<StridedSlicePrinter>();
