                                      ...
                                      const luci_interpreter::Tensor *tensor)
{
  ...
  {
    _hessian_data.recordHessian(node, tensor);
  }
}
 ...
 */
void HessianMap::unfold(std::vector<float> &buf, uint32_t input_n, uint32_t input_h,
                        uint32_t input_w, uint32_t input_c, uint32_t stride_h, uint32_t stride_w,
                        uint32_t dilation_h, uint32_t dilation_w, uint32_t kernel_oc,
                        uint32_t kernel_h, uint32_t kernel_w, uint32_t kernel_ic)
{
...
void HessianMap::recordHessian(const luci::CircleNode *node,
                               const luci_interpreter::Tensor *input_tensor)
{
  ...
  HessianVector &vectors = _hessian_map[node];
  vectors.update(hessian);
}
