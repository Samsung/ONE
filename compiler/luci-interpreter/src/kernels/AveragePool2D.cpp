...
AveragePool2D::AveragePool2D(const Tensor *input, Tensor *output, const AveragePool2DParams &params)
        ...
        for (int32_t channel = 0; channel < depth; ++channel)
        {
          ...
          for (int filter_y = filter_y_start; filter_y < filter_y_end; ++filter_y)
          {
            for (int filter_x = filter_x_start; filter_x < filter_x_end; ++filter_x)
            {
              ...
              sum += input_data[offset(input_shape, batch, in_y, in_x, channel)];
              filter_count++;
            }
          }
          const float average = sum / filter_count;
          output_data[offset(output_shape, batch, out_y, out_x, channel)] =
              activationFunctionWithMinMax(average, activation_min, activation_max);
        }
      }
    }
  }
    ...
    for (int32_t out_y = 0; out_y < output_height; ++out_y)
    {
      for (int32_t out_x = 0; out_x < output_width; ++out_x)
      {
        for (int32_t channel = 0; channel < depth; ++channel)
        {
          ...
          const int32_t average = (sum + filter_count / 2) / filter_count;
          output_data[offset(output_shape, batch, out_y, out_x, channel)] =
              activationFunctionWithMinMax(average, activation_min, activation_max);
        }
      }
