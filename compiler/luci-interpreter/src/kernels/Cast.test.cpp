           ...
           std::initializer_list<T2> output_data)
{
  ...
  CastParams params{};
  params.in_data_type = input_type;
  params.out_data_type = output_type;
  Cast kernel(&input_tensor, &output_tensor, params);
  kernel.configure();
  kernel.execute();
...
TEST(CastTest, TypesMismatch_NEG)
{
  ...
  CastParams params{DataType::S32, DataType::U8};
