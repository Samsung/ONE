...
{
  tflite::reference_ops::Relu6(input_shape, input_data, output_shape, output_data);
}

template <typename T>
static inline void ReluX(const tflite::ReluParams& params,
                         const tflite::RuntimeShape& input_shape, const T* input_data,
                         const tflite::RuntimeShape& output_shape, T* output_data)
{
  tflite::reference_ops::ReluX(params, input_shape, input_data, output_shape, output_data);
}
}
