...
#
# This script was copied from luci-value-test with adjusting arguments for circle-interpreter
#
...
for i in range(num_inputs):
    ...
    input_data.tofile(data_prefix + ".input" + str(i))
...
subprocess.run(
    [driver, circle_model, data_prefix + ".input", data_prefix + ".output"], check=True)
...
for idx in range(len(inpt_output_details)):
    ...
    try:
        ...
        if output_details["dtype"] in int_type:
        ...
        elif output_details["dtype"] in float_type:
            if np.allclose(
