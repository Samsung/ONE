...
namespace tflite
{
  ...
  namespace reference_ops
  {
          ...
          else
          {
            ...
            for (int j = 0; j < copy_size; ++j)
            {
              const int32_t value =
                  ...
                  output_zeropoint;
              output_ptr[j] =
                  static_cast<uint8_t>(std::max(std::min(255L, value), 0L));
            }
          }
        ...
        const RuntimeShape &output_shape, float *output_data)
    {
      ...
      const long int depth =
          MatchingDim(input_shape, trailing_dim, output_shape, trailing_dim);

      for (long int i = 0; i < outer_size; ++i)
      {
        for (long int c = 0; c < depth; ++c)
        {
          const int begin_input_c = std::max(0L, c - op_params.range);
                           ...
                           const RuntimeShape &output_shape, uint8 *output_data)
    {
      ...
      const int32 diff_min = params.diff_min;
      ...
      for (int i = 0; i < outer_size; ++i)
      {
        ...
        const int adjusted_diff_min =
            std::max(diff_min - (int32)1, // Note use of > below instead of >= above.
                     MultiplyByQuantizedMultiplierSmallerThanOneExp(
                                           ...
                                           int32 *lower_bound, int32 *upper_bound)
    {
      ...
      *lower_bound = std::max(*scaled_value / ((int32)1 << 10), (int32)0);
      *upper_bound =
