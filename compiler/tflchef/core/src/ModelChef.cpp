...
public:
  const char *GetBufferPointer(void) const
  {
    return reinterpret_cast<const char *>(_flatbuffer_builder->GetBufferPointer());
  }

  size_t GetSize(void) const { return _flatbuffer_builder->GetSize(); }
...
public:
  const char *base(void) const override { return _mc.GetBufferPointer(); }

  size_t size(void) const override { return _mc.GetSize(); }
