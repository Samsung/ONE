...
private:
  ...
  std::map<int32_t, BufferData_t> _buffer_data_map;
  std::map<int32_t, ::flatbuffers::Offset<::tflite::Buffer>> _buffer_offset_map;
      ...
      if (operand.has_make_sparse() && operand.make_sparse())
      {
        ...
        if (operand.type() == tflchef::FLOAT32)
        {
          ...
          if (_ext_offset)
          {
            ...
            _buffer_offset_map[buffer_index] = buffer;
          }
          else
          {
...
void ModelChef::cook_graph(const T &graph, std::map<std::string, int32_t> &symbol_table)
{
  ...
  // clear _tensor_vec, _operator_vec as these per graph and reused
  _tensor_vec.clear();
  _operator_vec.clear();
}
...
void ModelChef::finalize_ext_buffer(void)
{
  const char *buff_ptr = reinterpret_cast<const char *>(_flatbuffer_builder->GetBufferPointer());
  auto result = std::string(buff_ptr, _flatbuffer_builder->GetSize());
  ...
  for (auto &it : _buffer_data_map)
  {
    ...
    BufferData_t &buffer_data = it.second;

    auto buffer = std::string(it.second.begin(), it.second.end());
    int64_t offset = result.size();
    int64_t size = buffer_data.size();

    // auto &buffer_offset = _buffer_offset_map.at(index);
    tflite::Buffer *mutable_buffer = mutable_buffers->GetMutableObject(index);
