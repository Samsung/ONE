...
int entry(int argc, char **argv)
{
  ...
  const std::string fake_quant = "--fake_quantization";
  ...
  arser.add_argument(fake_quant)
    .nargs(0)
    .required(false)
    .help("Create fake-quantized model from the quantized model. NOTE: This feature will "
  ...
  if (arser[fake_quant])
  {
    options->enable(Algorithms::FakeQuantization);
  }
