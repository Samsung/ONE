...
std::vector<std::shared_ptr<LayerParam>> read_layer_params(std::string &filename)
{
  ...
  if (not ifs.is_open())
    throw std::runtime_error("Cannot open cfg file. " + filename);

  Json::CharReaderBuilder builder;
  JSONCPP_STRING errs;
  // Failed to parse
  if (not parseFromStream(builder, ifs, &root, &errs))
    throw std::runtime_error("Cannot parse cfg file. " + errs);

  auto default_type = root["default_quantization_dtype"];
